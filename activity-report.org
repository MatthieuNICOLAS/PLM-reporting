...
*** Questions
*** Planned
**** TODO Améliorer le fonctionnement du canvas
- Actuellement, à chaque application d'une opération (inverse ou non) sur le modèle du monde, le canvas est redessiné.
- Lorsque le code de l'utilisateur génère plusieurs milliers d'opération et que celles-ci sont appliquées au modèle, cela entraîne un freeze de l'onglet.
- Modifier la méthode de mise à jour du canvas pour qu'elle ne soit appelée qu'un nombre de fois limitée par seconde (30 ou 60 fps).
**** TODO Tester ngTranslate
