...
**** TODO Corriger les démos des 3 premiers exercices de la leçon des pancakes
- Les démos ne comportent pas les bonnes opérations
- Le résultat doit être plutôt déstabilisant pour les élèves
**** TODO Release la nouvelle version de webPLM
- [ ] Mettre à jour le numéro de version et tagger le commit correspondant
- [ ] Re-générer le binaire, le JAR et l'image docker de l'application
  - Ne pas oublier d'utiliser le bon token d'authentification pour GitHub
- [ ] Tagger l'image docker avec le bon n° de version de /webPLM/
- [ ] Redéployer les conteneurs tournant sur la machine de TELECOM Nancy
