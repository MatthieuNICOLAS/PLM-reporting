...
**** TODO Corriger les problèmes & bugs détectés lors du test en mode local
- [X] Manque *CLIENT_SECRET* de Google & GitHub
- [X] Réduire l'espace entre la barre du haut et le contenu de la page
- [X] Images des leçons inaccessibles
- [ ] Pas possible de récupérer son code en se connectant à son compte PLMAccounts
- [ ] Ajouter un écran de chargement durant
  - l'initialisation de PLM
  - l'authentification
  - la compilation du code de l'élève
- [ ] Voir pour supprimer RUNNING_PID à la fermeture du serveur
- [X] Le message indiquant que ce type de monde n'est pas encore supporté n'est plus affiché
- [X] Le slider permettant de choisir la vitesse d'animation a été oublié lors du refactoring de l'UI
