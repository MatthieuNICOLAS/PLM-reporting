...
- Ajouter les identifiants Git dans les comptes utilisateurs
  ...
  - Il faudrait que le service d'authentification soit en mesure de le récupérer pour l'associer au compte de l'utilisateur lors de sa création
    - [ ] soit l'id Git est transmis au client qui le retransmet au controller d'inscription
  ...
  - Reste à voir pour récupérer le code du nouvel utilisateur
    ...
    - Lorsqu'on change d'utilisateur, il faut donc
      - [ ] soit supprimer les instances de leçons existantes
...
- Un repo Git est créé à chaque connexion
  ...
  - Lorsqu'il s'identifie avec son compte (manuellement ou avec son cookie), on lui réattribue son repo Git habituel
  - Que faire du repo Git momentanément utilisé?
    - Le garder et envoyer la trace sur le repo *GitHub*
      - Le marquer en tant que *momentané* à l'aide d'un fichier/d'un token dans l'id?
...
**** TODO Ajouter les identifiants Git dans les comptes utilisateurs
...
**** TODO Corriger changement de langue et selection d'exos
...
**** TODO Finaliser le script redémarrant webPLM
