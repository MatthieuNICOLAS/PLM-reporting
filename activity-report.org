...
- Re-implémenter les tests unitaires
  ...
  - Implémentation des tests unitaires pour *ExerciseRunner* pour chaque *ProgrammingLanguage*
    - Implémentation des tests concernant l'exécution du code en Java
    - Voir comment factoriser ces tests pour chaque *ProgrammingLanguage*
*** Planned
...
**** DONE Retirer *Game*
** Semaine du <2016-02-22 lun.> au <2016-02-26 ven.>
*** Done
- Re-implémenter les tests unitaires
  - Correction des tests unitaires de /webPLM/
    - Le modèle *PLM* n'existe plus, les tests unitaires pour cette classe ont donc été retirés
    - Correction des tests unitaires de *BuggleWorld*
  - Implémenter des tests unitaires pour *ExerciseRunner* pour chaque *ProgrammingLanguage*
    - Utilisation de /@RunWith(Parameterized.class)/ pour que la classe de test *ExerciseRunnerTest* soit instanciée successivement avec chaque langage de programmation
    - Modification des tests pour adapter le code exécuté en fonction du langage de programmation actuel
- Re-implémenter la gestion des indices
  - Les indices sont directement présents dans les consignes d'un exercice
  - Y a-t-il un intérêt à les parser depuis /PLM/ et à les repertorier dans le modèle d'un *Exercise*?
    - Je ne vois un intérêt à cela qu'uniquement pour effectuer un sanity check, pour vérifier lors du déclenchement d'un évènement git /readTip/ que l'indice existe bel et bien
  - On souhaite en tout cas permettre au développeur de surcharger le processus de parsage des indices pour qu'il puisse modifier facilement le format affiché
  - Création de *TipFactory*
    - Nécessite un *Exercise*, une mission et une *Locale*
    - Extrait les tips
    - Les ajoute à la liste des tips de *Exercise*
    - Génère une nouvelle mission où les tips sont cachés jusqu'à ce que l'utilisateur demande à les consulter
  - Ajout d'une directive /tips/ permettant d'afficher sous la forme de sections dépliables les indices
  - Surcharge de *TipFactory* pour que la mission générée utilise cette directive au lieu de générer un lien
- Implémenter *ToJSON* & Implémenter un constructeur à partir d'un *JSONObject*
  - Univers des tests unitaires
    - Comment exporter la liste des paramètres (un tableau d'*Object*) en JSON et la ré-importer correctement dans le constructeur?
    - Vu que *JSON.Simple* convertit de base les *int* en *Long*, ne risque-t-on pas de rencontrer des problèmes dans les exercices où les paramètres sont des *int*?
    - Dans certains exercices, notamment ceux de la leçon /recursion.cons/, on passe des objets *RecList* en paramètre
    - Modification de /BatTest.toJSON()/
      - Le champ /parameters/ est un *JSONArray* de *JSONObject*
      - Chaque *JSONObject* comporte deux champs
      - /parameter/ qui est le paramètre
      - /type/ qui est le nom de la classe du paramètre
      - Ceci permettra, en fonction du /type/, de traiter les cas particuliers des paramètres (tels que les instances *RecList*)
    - Modification de /BatTest(JSONObject json)/
      - Lors du parcours de la liste des paramètres, vérifie la valeur du champ /type/
      - Permet de traiter les cas particuliers
    - Plutôt que de faire ça à la main à chaque fois que nous en avons besoin, il serait sans doute préférable d'utiliser une librairie de sérialisation/désérialisation JSON pouvant le gérer automatiquement
- Cloner les mondes, entités et opérations lors d'un clonage d'un exercice
  - Ajout d'un appel à /World.copy()/ dans le constructeur
  - Besoin de modifier le constructeur par copie de *World* pour copier aussi les opérations (afin de conserver la démo)
  - Besoin d'ajouter une méthode /clone()/ pour toutes les classes héritant de *Operation*
- Passer à une librairie prévue pour la sérialisation/désérialisation au format JSON
  - /Jackson/
    - Du côté de /Jackson/, j'ai trouvé plusieurs liens intéressants
      - Comment ajouter le type automatiquement au JSON pour que le désérialiseur puisse reformer correctement l'objet:  http://www.davismol.net/2015/03/05/jackson-json-deserialize-a-list-of-objects-of-subclasses-of-an-abstract-class/
      - Comment lire soi-même le type pour orienter la désérialisation: http://stackoverflow.com/questions/28384281/can-jackson-determine-root-object-type-to-deserialize-to-when-json-includes-type
    - Sérialisation de *Exercise* en utilisant /Jackson/
      - Suppression de boucles infinies
	- *World* -> *Entity* -> *World*
	- Ajout de l'annotation /@JsonManagedReference/ dans *World* et /@JsonBackReference/ dans *Entity*
      - Ajout de getters dans *World*
	- /getInitialWorld()/
	- /getAnswerWorld()/
      - Suppression des champs doublons dans les entités
	- Les traductions des getters des entités sont considérées par /Jackson/ comme de nouveaux champs
	- Ajout de l'annotation /@JsonIgnore/
      - Ajout du type des objets
	- Utilisation de l'annotation /@JsonTypeInfo/
	- On rencontre cependant le problème des entités solutions ayant pour type *plm.runtimeX.NomExercise*
	- Pour le moment, je parcours le *JsonNode* obtenu par /Jackson/ de l'objet et modifie les valeurs des champs /type/ des entités solutions
      - Ajout de *CustomColorSerializer* pour sérialiser les *Color*
	- Par défaut, la sérialisation des *Color* par /Jackson/ est exhaustive
	- Se contente de sérialiser les valeurs RGBA de *Color* dans notre cas
*** Planned
**** DONE Re-implémenter les tests unitaires
...
- Les ré-implémenter
  ...
  - [X] Implémenter des tests unitaires pour *ExerciseRunner* pour chaque *ProgrammingLanguage*
    - [X] /run()/ doit returner un *ExecutionProgress* avec /outcome=OutcomeKind.COMPILATION/ si le code contient une erreur de syntaxe
    - [X] /run()/ doit returner un *ExecutionProgress* avec /outcome=OutcomeKind.FAILED/ si le code génère une erreur d'exécution
    - [X] /run()/ doit returner un *ExecutionProgress* avec /outcome=OutcomeKind.FAILED/ si le code ne permet pas d'atteindre l'état objectif
    - [X] /run()/ doit returner un *ExecutionProgress* avec /outcome=OutcomeKind.TIMEOUT/ si le code ne finit pas de s'exécuter avant la durée fixée
    - [X] /run()/ doit returner un *ExecutionProgress* avec /outcome=OutcomeKind.PASS/ si le code permet d'atteindre l'état objectif
- Revoir si les tests de git ont aussi besoin d'être mis à jour
**** DONE Re-implémenter la gestion des indices
- Auparavant, ils étaient gérés dans *Lecture*
- Les regénérer depuis *ExerciseFactory* à l'initialisation
** Semaine du <2016-02-29 lun.> au <2016-03-04 ven.>
*** Done
- Passer à une librairie prévue pour la sérialisation/désérialisation au format JSON
  - Je rencontre toujours des difficultés à sérialiser et désérialiser au format JSON *Exercise* avec la libraire /Jackson/
  - Création d'un projet de test
    - Recréé l'architecture des composants avec *World*, *Entity* et leurs implémentations respectives *BuggleWorld*, *Buggle*
  - /@JsonManagedReference/ et /@JsonBackReference/ ont l'air d'aider à la sérialisation en empêchant les boucles infinies
  - Mais ces annotations n'ont pas l'air d'ajouter d'information pouvant aider à la désérialisation
  - Remplacer ces annotations par /@JsonIdentityInfo(generator=ObjectIdGenerators.IntSequenceGenerator.class, property="@jsonId")/ sur *World* semble plus efficace
  - Cette annotation permet d'attribuer un identifiant pour chaque instance de *World*
  - Lors de la sérialisation des autres objets ayant une référence vers un monde, /Jackson/ se contentera de spécifier l'identifiant de l'instance de *World* concernée
  - Succès /troublant/ de la sérialisation/désérialisation de *World* du projet de test
    - Initialisation d'un monde initial
      - Création d'une instance /world/ de *BuggleWorld*
      - Création d'instances de *Buggle* ayant une référence vers /world/
      - Ajout des entités créées à /world/
    - Sérialisation du monde
      - /{"type":"model.BuggleWorld","@jsonId":1,"id":"World 1","name":"Essai","entities":[{"type":"model.Buggle","world":1,"id":"Entity 1"},{"type":"model.Buggle","world":1,"id":"Entity 2"}]}/
    - Désérialisation du JSON pour générer une instance /clone/ du monde
      - L'instance générée correspond au monde initial
      - /{"type":"model.BuggleWorld","@jsonId":1,"id":"World 1","name":"Essai","entities":[{"type":"model.Buggle","world":1,"id":"Entity 1"},{"type":"model.Buggle","world":1,"id":"Entity 2"}]}/
    - Cependant, le constructeur appelé par /Jackson/ n'est pas censé gérer les entités
      - Il se contente d'extraire l'/id/ et le /name/ du JSON et de les affecter à la nouvelle instance
    - Les entités sont donc ajoutées automatiquement par un mécanisme de /Jackson/
    - Je n'arrive cependant pas à comprendre son workflow
  - Même constat avec les instances de *Cell*
    - Ajout d'un paramètre /private Cell[][] cells/ à *World*
    - Génération de cellules par défaut dans le constructeur de *World*
    - Modification d'une des cellules avant de sérialiser /world/
    - La cellule de l'objet /clone/, /clone/ ayant été obtenu par désérialisation du JSON, comporte bien la modification effectuée
    - Alors que le constructeur appelé par /Jackson/ aurait dû généré un tableau de cellules toutes identiques à la cellule par défaut
    - Il semblerait donc que /Jackson/, après avoir généré l'instance /clone/ à l'aide du constructeur indiqué, génère les objets ayant une reférence vers celui-ci et les bind correctement automatiquement
  - Modification du code de /PLM/ pour
    - Utiliser l'annotation /@JsonIdentityInfo(generator=ObjectIdGenerators.IntSequenceGenerator.class, property="@jsonId")/
    - Retirer les arguments /initialWorld/, /answerWorld/ et /cells/ des constructeurs utilisés par /Jackson/
  - Arrive enfin à sérialiser puis désérialiser un *Exercise*
    - On sérialise un *Exercise* standard tel que *Environment*
    - On remplace les valeurs des champs /type/ des entités des mondes solutions par celles des champs /type/ des mondes initiaux
      - Sinon on aurait une erreur à la désérialisation car classe inconnue
    - On sérialise l'objet
    - On désérialise le JSON obtenue en un *BlankExercise*
    - On initialise /currentWorld/ en clonant /initialWorld/
    - On définit un /progLang/ et un /humanLang/ courant
  - Ajout de la config /Jackson/ pour l'univers des tests unitaires
  - Ajout de *JSONUtils*
    - Classe mettant à disposition des méthodes /static/ pour générer du JSON à partir d'objet Java
*** Planned
** Semaine du <2016-03-08 mar.> au <2016-03-11 ven.>
*** Done
- Passer à une librairie prévue pour la sérialisation/désérialisation au format JSON
  - Modification du code de /webPLM/ pour utiliser les méthodes de *JSONUtils* pour fournir l'exercice et les opérations au client
  - Adaptation du code client de /webPLM/ au nouveau format du JSON
- Corriger les bugs d'interfaces pour la présentation de Martin aux profs d'ISN
  - Issue #24
    - L'idée serait de générer une méthode de callback affichant le résultat lorsqu'on reçoit ce dernier
    - Cette méthode ne serait appelée qu'à la fin de l'animation
    - Ajout d'une variable /exercise.animationOnGoing/ spécifiant si l'animation est en cours ou non
    - Attachement de la méthode de callback à cette variable à l'aide de /$watch/, fournie par /AngularJS/
      - Permet de déclencher la méthode de callback à chaque modification de cette variable
      - Il faut faire attention cependant, la callback est appelée dès la création du listener
  - Issue #71
    - Actuellement, on se contente d'envoyer un message au serveur pour lui dire de stopper l'exécution et de ne plus transférer les opérations
    - Arrêter l'animation serait utile
    - Ajout de /exercise.executionStopped/ indiquant si l'exécution courante a été interrompue par l'utilisateur
    - Arrêt des boucles d'animations lors de l'appel à /stopExecution()/
    - Lors de la réception du résultat, vérifie si l'exécution a été stoppée pour savoir si on délaie l'apparition du message ou pas
  - Issue #75
    - Je me suis contenté de retirer la mention au graphe de progression pour le moment
  - Issue #79
    - Les noms des mondes dans le fichier de cache de la démo de *TricolorHanoi3* ne correspond pas aux nom des mondes de l'exercice
    - Il semblerait que j'ai effectué un mauvais copier/coller lors du refactoring de l'univers de Hanoi (voir ce commit https://github.com/BuggleInc/PLM/commit/a454d2fb609ae97e9e82fc89afc9cf24b30876b3#diff-e6efca3e2b225b61f0357a30a70aa9cd)
    - Correction des noms des mondes dans /PLM/ et mise à jour de la version de /PLM/ embarquée dans /webPLM/
  - Issue #58
    - Ce problème a déjà été résolu dans les branches de refactoring par les commits [[https://github.com/BuggleInc/webPLM/commit/eedc3cdadf6a2908f14b0bad88151ba3dd827a1a][eedc3cdadf6a2908f14b0bad88151ba3dd827a1]]a et [[https://github.com/BuggleInc/PLM/commit/09be922fef2d4fd8d3cadab5b2737f8c09c860ca][09be922fef2d4fd8d3cadab5b2737f8c09c860ca]]
    - Je serai bien tenté de /cherry-pick/ ces commits mais une partie de leurs modifications ne sont pas compatibles avec la version actuelle de master
    - En attendant de fusionner les modifications de la branche /refactor-lessons-exercises/, je me suis contenté de copier/coller les éléments de ces commits qui étaient pertinents
  - Issue #2
    - Même cas que l'issue #58, issue déjà corrigée par des commits pas encore mergés [[https://github.com/BuggleInc/webPLM/commit/a07f2a7d781d59b652a49bebce6360c368c998cf][a07f2a7d781d59b652a49bebce6360c368c998cf]] et [[https://github.com/BuggleInc/webPLM/commit/d1ebbb00b93dabfeab27a293bdfafd3af11c3b2d][d1ebbb00b93dabfeab27a293bdfafd3af11c3b2d]]
    - Copier/coller des éléments nous intéressant dans ces commits
  - Pull Request #80
    - Ajout de commentaires concernant le code en lui-même
    - Ajout d'une réponse plus complète sur un problème détecté concernant l'implémentation actuelle : https://github.com/BuggleInc/webPLM/pull/80#issuecomment-194839065
    - On ne va donc pas intégrer cette PR pour le moment
  - Issue #65
    - Ce détail peut facilement être résolu: http://foundation.zurb.com/sites/docs/v/5.5.3/components/offcanvas.html#optional-javascript-configuration
    - En fait, ce n'est pas si trivial que cela
    - Cette option permet effectivement de fermer le menu latéral dès qu'on clique sur un de ses items
    - Mais certains des items du menu sont des menus déroulants
    - Il ne faudrait pas que le menu se ferme si l'on clique sur un de ces items particuliers
    - Il est possible de déclencher la fermeture manuellement avec la commande /$('.off-canvas-wrap').foundation('offcanvas', 'toggle', 'move-right');/
    - Je ne suis toujours pas convaincu du résultat
    - Ajouter une préférence utilisateur permettant à chacun de définir le comportement à adopter?
  - Issue #77
    - Ajout du plugin SBT assembly: https://www.playframework.com/documentation/2.4.x/Production#Using-the-SBT-assembly-plugin
    - Rencontre de conflits lors de la génération du jar, certains packages sont référencés par 2 dépendances différentes
      - Généralement, ces packages sont référencés d'une part par /PLM/ et de l'autre par /webPLM/
    - Ajout d'une nouvelle configuration de build pour générer un jar de /PLM/ sur-mesure pour /webPLM/ pour ne pas avoir de doublons dans les dépendances
    - Il reste un problème de dépendance généré par /Play/
      - /commons-logging/ et /jcl-over-slf4j/ rentrent en conflit
    - Conflit connu, une solution est proposé par /Play/ pour le contourner
    - Cette solution n'a pas l'air de fonctionner
    - Ajout d'une /MergeStrategy/ spécifique pour conserver la version la plus récente
  - Issue #69
    - La manière pour récupérer le token d'authentification auprès de GitHub n'avait pas été mise à jour depuis la disparition de l'objet *Global*
    - Utilise maintenant la variable de configuration pour récupérer le token
*** Planned
**** DONE Release l'application sous forme de .jar
- Voir [[https://www.playframework.com/documentation/2.4.x/Production][cette page]] pour configurer le build pour générer un .jar
**** DONE Corriger les bugs d'interfaces pour la présentation de Martin aux profs d'ISN
- [X] https://github.com/BuggleInc/webPLM/issues/75: Progression graphs should be implemented or not mentionned
- [X] https://github.com/BuggleInc/webPLM/issues/79: Demo empty in the last exercise of Tri-color Hanoi Towers
- [X] https://github.com/BuggleInc/webPLM/issues/71: An infinite loop loops infinitely (stop button is useless)
- [X] https://github.com/BuggleInc/webPLM/issues/58: Hints do not seem to be implemented
- [X] https://github.com/BuggleInc/webPLM/issues/24: When you pass an exercise, the congratulation picture comes before the end
- [X] https://github.com/BuggleInc/webPLM/issues/2: Objectives tab should not show the demo by default 
- [X] https://github.com/BuggleInc/webPLM/issues/69: Reporting a bug does not work
- [-] https://github.com/BuggleInc/webPLM/issues/65: When switching the exercise, the chooser tab should close automatically
- [-] https://github.com/BuggleInc/webPLM/pull/80: Add localStorage
- [X] https://github.com/BuggleInc/webPLM/issues/77: The local server should be a single jarfile
** Semaine du <2016-03-14 lun.> au <2016-03-18 ven.>
*** Done
- Corriger les démos des 3 premiers exercices de la leçon des pancakes
  - Ce bug est dû au fait que les mondes sont générés aléatoirement
  - Comme on met en cache les démos au préalable, les démos ne correspondent pas aux mondes affichés
  - On peut
    - Rendre fixe les mondes, en supprimant leur composante aléatoire
    - Supprimer le système de cache des démos (ce qui est déjà fait dans la branche refactoring)
  - Si on conserve le côté aléatoire de ces mondes, il faut pouvoir
    - Transmettre l'état initial des mondes aux juges pour qu'ils exécutent le code de l'élève dans le même environnement (ce qui est déjà fait dans la branche refactoring)
    - Garder une trace dans git de l'état initial des mondes pour pouvoir rejouer les traces de façon cohérente
  - Suppression de la composante aléatoire des mondes de ces exercices
  - Mise à jour des démos pour correspondre aux nouvelles instance des mondes
- Release la nouvelle version des juges
  - Mise à jour de la version de PLM embarqué dans les juges
  - Release d'une nouvelle image docker /buggleinc/plm-judge:1.3.0/
    - Ajout d'un tag pour le numéro de version pour qu'on sache avec quelle version de /webPLM/ on souhaite utiliser ces juges
  - Re-déploiement des juges sur la machine de TN
  - Cela n'a pas corrigé le problème
  - Il semblerait que les machines sur lesquelles on a déployé les juges l'été dernier sont toujours up
    - corosif
    - penny
  - Re-déploiement des juges sur ces machines
  - Le problème semble résolu
- Fixer les juges
  - Lors de la mise à jour des juges, j'ai exporté la mauvaise version de PLM
  - La version utilisée par les juges essayait alors de récupérer et de stocker la progression de l'élève dans git
  - Le code de l'élève était correctement exécuté, mais trop tardivement pour que /webPLM/ accepte la réponse
  - Merge des modifications de PLM/master dans PLM/judge
  - Export de cette version de PLM dans les juges
  - Re-génération de l'image docker des juges et re-déploiement des conteneurs
- Trouver un moyen de récupérer la date de la dernière version du code de l'élève que l'on stocke
  - Avec /JGit/, on peut "facilement" retrouver les commits concernant un fichier avec *LogCommand*
    - /log().addPath(path)/ permet de filtrer les commits en fonction du fichier qui nous intéresse
    - /log().setMaxCount(nbCommits)/ permet de filtrer le nombre de commit que l'on souhaite récupérer
  - A partir du moment que l'on récupère le commit qui nous intéresse, on peut retrouver sa date
    - /commit.getAuthorIdent().getWhen()/
  - Reste à voir si
    - [ ] j'implémente moi-même cette portion de code et je me contente d'expliquer aux élèves comment ils peuvent dorénavant récupérer la date de la version du serveur depuis /webPLM/
    - [ ] c'est plus intéressant pédagogiquement de les laisser travailler sur le sujet, je me contente de répondre à leurs questions
- Délayer l'affichage du résultat
  - Ajout d'un /$timeout()/ pour que le résultat ne s'affiche que 500ms après la fin de l'animation
- Scroller automatiquement sur la vue du monde lors de l'exécution du code de l'élève
  - /$anchorScroll(elementID)/ permet de faire scroller la vue sur /elementID/ via le code
  - Ajout de /$anchorScroll(exercise.drawingArea)/ dans /runCode()/
  - Le scroll se fait de façon plutôt abrupte
    - Instantanément, la vue se retrouve centrée sur la représentation du monde
  - J'ai essayé d'utiliser plutôt des librairies pour disposer d'un smooth scroll
    - https://github.com/d-oliveros/ngSmoothScroll
    - https://github.com/arnaudbreton/angular-smoothscroll
  - Mais j'ai pas réussi à faire fonctionner l'ensemble
*** Planned
**** DONE Corriger les démos des 3 premiers exercices de la leçon des pancakes
- Les démos ne comportent pas les bonnes opérations
- Le résultat doit être plutôt déstabilisant pour les élèves
**** DONE Release la nouvelle version de webPLM
- [X] Mettre à jour le numéro de version et tagger le commit correspondant
- [X] Re-générer le binaire, le JAR et l'image docker de l'application
  - Ne pas oublier d'utiliser le bon token d'authentification pour GitHub
- [X] Tagger l'image docker avec le bon n° de version de /webPLM/
- [X] Redéployer les conteneurs tournant sur la machine de TELECOM Nancy
**** DONE Release la nouvelle version des juges
- On a modifié les exercices de la leçon /sort.pancake/ pour ne plus générer les mondes aléatoirement
- Il faut donc mettre à jour la version de PLM contenu dans les juges
- Mettre à jour l'image docker /buggleinc/plm-judge/
- Re-déployer les conteneurs
**** DONE Fixer les juges
- Ceux-ci ne répondent pas plus fréquemment que d'habitude
- Trouver l'origine de ce dysfonctionnement
**** DONE Délayer l'affichage du résultat
- L'affichage du résultat se fait maintenant dès que l'animation de la représentation du monde s'achève
- Les utilisateurs n'ont pas le temps de constater qu'ils sont parvenus à l'état objectif que le résultat s'affiche
- Ajouter un léger délai
**** DONE Scroller automatiquement sur la vue du monde lors de l'exécution du code de l'élève
- Lorsqu'un utilisateur soumet son code, la représentation du monde s'anime pour lui montrer quelles opérations s'effectuent
- Cependant, la représentation du monde n'est pas forcément visible à ce moment, en fonction de où l'utilisateur se place dans le panel de gauche
- Scroller automatiquement vers la représentation du monde peut s'avérer ergonomique
** Semaine du <2016-03-24 jeu.> au <2016-03-25 ven.>
*** Done
- Retravailler l'UI de PLM
  - Ajout du mock-up de Jérémie dans le repo de webPLM: https://github.com/BuggleInc/webPLM/blob/master/mockup-UI/PLM.pdf
- Passer à une librairie prévue pour la sérialisation/désérialisation au format JSON
  - Ajout du support de l'univers des tests unitaires
    - Modification du nom de certains champs du JSON formé par Jackson pour correspondre au format attendu par le code JS côté client
    - Suppression des champs /displayedResult/ et /displayedExpected/
    - Serait-il pas préférable de déplacer le code traitant /parameters/, /result/ et /expected/ pour les afficher correctement en fonction du langage côté client? 
  - Mise à jour des juges pour utiliser Jackson
    - Ajout de Jackson
    - Modification de *RequestMsg* pour parser le JSON et extraire les différentes valeurs des champs
    - Modification de *BasicListener* pour utiliser les méthodes de *JSONUtils* pour formatter les opérations et les envoyer
*** Planned
** Semaine du <2016-03-29 mar.> au <2016-04-01 ven.>
*** Done
- Passer à une librairie prévue pour la sérialisation/désérialisation au format JSON
  - *ExecutionProgress*
    - Ajout de la config Jackson
    - Attention: ne transmet pas le *ProgrammingLanguage*
  - *JSONUtils*
    - Modification de /jsonToExercise/ pour générer un *Exercise* à partir d'un *JsonNode* au lieu d'une *String*
    - Modification de /createMessage/ pour placer le champ /cmd/ en tête du JSON généré
  - *Judge*
    - Suppression des classes inutiles *OperationParser*, *StreamMsg*
  - *TribunalActor*
    - Récupère le /executionResult/ renvoyé par le juge
    - Y ajoute le /currentProgLang/
      - Nécessaire pour ensuite stocker correctement le code de l'élève dans *GitActor*
    - Correction d'un bug ne stoppant pas la boucle d'écoute du juge lorsqu'on reçoit /executionResult/
      - Ne modifiait pas /state/
      - On continuait donc d'écouter la queue de message jusqu'au moment où l'exécution "timeout"
      - On retournait un *ExecutionProgress* correspondant à un "timeout" à *PLMActor*
	- Sauf que *PLMActor* n'attendait plus de réponse
	- Il avait généralement déjà reçu et traité l'/executionResult/ produit par le juge
  - Merge de la branche /jackson/ dans la branche /refactor-lessons-exercises/
- Déplacer les exercices de PLM à webPLM
  - Maintenant que Jackson est implémenté et testé pour l'univers des Buggles et des tests unitaires, on peut adapter les exercices de ces types
  - Ecriture d'un script permettant de
    - Parser le JSON d'une leçon pour déterminer les exercices qui n'ont pas encore été déplacé
    - Pour chaque exo
      - Copier l'ensemble des fichiers nécessaires de cet exo de PLM à webPLM
    - Mise à jour du JSON de la leçon
  - Reste à modifier manuellement les fichiers
    - package
    - constructeur de l'exercice
    - constructeurs des mondes
  - Problème avec *BatExercise*, dans leur cas, il faut aussi
    - Recréer correctement les différentes entités à partir des instructions /templatePython/, /templateScala/ et du code java contenu dans l'exercice
    - /extends ExerciseTemplated/ au lieu de /extends BatExercise/
  - Portage d'exercices de /welcome.bat.bool1/
*** Planned
** Semaine du <2016-04-04 lun.> au <2016-04-07 jeu.>
*** Done
- Vérifier le problème rencontré par Ariel
  - Ce bug semble avoir été résolu y a quelques temps: https://github.com/BuggleInc/PLM/pull/474
  - Je n'ai pas réussi à rencontrer la même erreur de compilation qu'Ariel, que ça soit sur la branche /master/ ou /javaUI/
  - Envoi d'un mail à Ariel pour vérifier la version du projet qu'il compile et m'assurer qu'il est à jour
- Ecriture d'alias pour nettoyer Docker
  - Pour supprimer l'ensemble des containers non-lancés: /docker rm $(docker ps -aqf status=exited)/
  - Pour supprimer l'ensemble des images sans tag: /docker rmi $(docker images | grep '^<none>' | tr -s ' ' | cut -d ' ' -f 3)/
- Déplacer les exercices de PLM à webPLM
  - Portage de *Moria* et des *Traversals*
  - Rencontre d'un problème lors du portage de *Island*
    - La désérialisation d'un tableau d'objets simples par Jackson fonctionne correctement
    - Mais la désérialisation d'un tableau de tableaux d'objets pose problème
      - On obtient un tableau de listes d'objets
      - On a besoin de reconvertir les listes en tableaux pour éviter des problèmes de cast lors de l'exécution
  - Portage de *Island*
  - Portage de *RandomMouseMaze*
  - Rencontre d'un problème lors du portage de *WallFollowerMaze*
    - La consigne de cet exercice comporte un indice
    - Cet indice explique par le biais d'images à l'apprenant la méthode qu'il doit implémenter
    - Il faut donc rendre accessible ces images
- Débugger la désérialisation des paramètres
  - Ajout du type des paramètres au JSON
    - Permet à Jackson de recréer les bonnes structures de données à la désérialisation
  - Modification du constructeur de *BatTest* côté client
    - Le JSON reçu pour /parameters/ a été modifié à cause de l'ajout du type
      - Pour un exemple donné /parameters/, un tableau d'*Object* comportant un *int[]*, une *ArrayList<Integer>* et un *int*
      - Auparavant, on avait le format suivant:
        - parameters: [ 
                        [1, 2, 3], 
                        [4, 5, 6],
                        7 
      ...
      - Maintenant, on a:
        - parameters: [
                        [ "[I", [1, 2, 3] ],
                        [ "java.util.ArrayList", [4, 5, 6] ],
                        7
                      ]
    - Pour chaque /parameter/ de /parameters/, si /parameter/ est un tableau alors on sait que
      - /parameter[0]/ est le type du paramètre
      - /parameter[1]/ est le paramètre
    - On se contente donc de régenérer la liste des paramètres, sans leur type, en filtrant de cette manière
- Gérer les images présentes dans les consignes d'exercices
  - Pour rendre accessible les fichiers d'un répertoire via URL dans Playframework, il faut ajouter une commande dans le fichier de build
    - unmanagedResourceDirectories in Assets += baseDirectory.value / "exercises"
  - En modifiant les règles de routing utilisées, on peut dorénavant exposer ces fichiers
    - Ajout de: GET     /exercises/*file                     controllers.Assets.at(path="/public", file)
  - Cette solution présente quelques défauts
    - L'ensemble des fichiers présents dans /exercises/ sont copiés dans /public/ lors du build
      - Les images
      - Mais aussi les entités
    - Les solutions sont donc aussi rendues accessibles de cette manière
    - De plus, les entités sont déjà présentes dans /unmanagedSources/, elles sont donc dupliquées inutilement
  - On peut modifier la règle de routing pour n'exposer que les images pour empêcher d'accéder aux entités
    - Exemple: GET     /exercises/$file<.+\.png$>          controllers.Assets.at(path="/public", file)
  - En cherchant un moyen d'éviter de dupliquer les entités, je me suis aperçu que certains répertoires sont manquants de l'archive générée
    - /exercises/
      - C'est en parcourant ce répertoire qu'on génère dynamiquement la liste des exercices globale en fonction des classes présentes
    - /lessons/
      - C'est à partir des fichiers /main.json/ contenu dans ce repertoire qu'on génère les objets *Lesson* qui répertorie la liste des exercices composant chaque leçon
*** Planned
**** DONE Nettoyer docker
- Par défaut, docker conserve l'ensemble des containers lancés et des images construites
- Voir pour supprimer les containers et les images plus utilisés
**** DONE Débugger la désérialisation des paramètres
- Lorsqu'on sérialise en utilisant Jackson un *Object[]*, la sérialisation obtenue est ambigüe
- Par exemple, sérialiser un *int[][]* ou un *ArrayList<Integer>[]* mène au même résultat
  - parameters: [ 
                  [1, 2, 3], 
                  [4, 5, 6]
                ]
- Lorsque Jackson désérialise ce JSON, il choisit de générer un *ArrayList<Integer>[]* ce qui ne nous convient pas dans tous les cas
- Dans *Buggle's Island*, on attend par exemple un *int[][]*
- Voir comment lever l'ambigüité lors de la sérialisation/désérialisation des paramètres de *BatTest*
** Semaine du <2016-04-19 mar.> au <2016-04-22 ven.>
*** Done
- Ajouter le répertoire /lessons/ à l'archive générée
  - [ ] Ajouter /lessons/ au /unmanagedResourceDirectories/
    - Ajout de unmanagedResourceDirectories in Compile += baseDirectory.value / "lessons" dans /build.sbt/
    - Cela se traduit par l'ajout du contenu de /lessons/ dans le répertoire /conf/
    - Ce résultat me convient moyennement
      - Je ne considère pas les leçons comme des fichiers de config
  - [ ] Ajouter /lessons/ au /assets/
    - Ajout de unmanagedResourceDirectories in Assets += baseDirectory.value / "lessons" dans /build.sbt/
    - Cela se traduit par l'ajout du contenu de /lessons/ dans le répertoire /public/
    - Ce résultat me convient moyennement aussi
      - Je ne considère pas que les fichiers correspondant aux leçons doivent être public
      - Ils sont uniquement à utiliser côté serveur
  - [X] Placer le répertoire /lessons/ dans le répertoire /dist/
    - Les fichiers et dossiers présents sont automatiquement à l'archive
    - Cela se traduit par l'ajout du répertoire /lessons/ à la racine de l'archive
    - C'est la solution qui me paraît la moins pire
    - Besoin d'ajuster le chemin vers les fichiers des leçons en fonction qu'on soit en mode production ou dev
      - ~play.Play.isProd~ permet de récupérer cette information
    - Mise à jour de *LessonsActor* pour gérer les chemins différents des fichiers en mode production ou dev
- Ajouter le répertoire /exercises/ à l'archive générée
  - Comme pour les leçons, j'ai ajouté le répertoire /exercises/ dans le répertoire /dist/
  - Le répertoire est cependant toujours ajouté aux sources pour pouvoir avoir accès aux classes des exercices
    - unmanagedSourceDirectories in Compile += baseDirectory.value / "dist" / "exercises"
  - Mise à jour de *ExercisesActor* pour gérer les chemins différents des fichiers en mode production ou dev
- Remplacer les classes des exercices par leur équivalent JSON
  - Implémentation d'une moulinette pour générer le JSON
    - Instancie chaque exercice via leur constructeur et *ExerciseFactory*
    - Ecrit dans des fichiers leur représentation JSON
      - ~JSONUtils.mapper.writeValue(new File(path + ".json"), JSONUtils.exerciseToJudgeJSON(exercise))~
  - Ajout de la méthode /fileToExercise()/ à *JSONUtils*
    - Génère une instance *BlankExercise* à partir du JSON d'un exercice
  - Modification de *ExercisesActor* pour utiliser /fileToExercise/ pour instancier les exercices
*** Planned
**** DONE Ajouter le répertoire /lessons/ à l'archive générée
- Il s'avère que ce répertoire n'est pas inclus lors du build
- Il est nécessaire pour générer les objets *Lesson*
- L'archive (de la branche /refactor-exercises/) actuellement générée crash donc au lancement car incapable d'instancier les leçons
- Modifier la configuration du build pour inclure ce répertoire
- Réfléchir à une alternative pour gérer les leçons (modules?)
**** DONE Ajouter le répertoire /exercises/ à l'archive générée
- De la même façon, /exercises/ n'est pas inclus dans le build
- Les exercices ne sont donc pas présent dans l'archive finale
**** DONE Remplacer les classes des exercices par leur équivalent JSON
- Corriger le problème des exercices manquants dans l'archive a permis de se rendre compte d'un autre problème
- Certains exercices génèrent leurs monde à partir de représentations contenus dans des fichiers .map
- Les chemins vers ces fichiers sont donc incorrects selon qu'on soit en mode prod ou mode dev
- Les exercices doivent à mon sens être découplés de l'architecture de webPLM, ils ne devraient pas la connaître et résoudre eux-même ce problème de chemin
- Stocker au format JSON les exercices pour les instancier à partir de leur représentation JSON en utilisant Jackson permettrait de résoudre ce problème
** Semaine du <2016-04-25 lun.> au <2016-04-29 ven.>
*** Done
- Implémenter *ToJSON*
  - On est passé à Jackson depuis pour sérialiser/désérialiser les exercices
  - L'interface *ToJSON* a été supprimée car devenu inutile et redondante
- Implémenter un constructeur à partir d'un *JSONObject*
  - De la même façon, nous utilisons Jackson pour instancier un objet à partir de son JSON dorénavant
  - Nous ne manipulons donc plus de *JSONObject* et n'avons plus besoin d'ajouter un constructeur correspondant
- Configurer Jackson pour générer différents JSON en fonction de l'utilisation et du destinataire
  - [ ] Utiliser les /@JsonView/
    - Permet de spécifier différents niveaux de visibilités des champs par le biais d'annotations
    - En fonction du niveau de visibilité attaché à l'*ObjectMapper* lors de la sérialisation/désérialisation de l'objet, les champs sont pris en compte ou non
    - Permet pas de spécifier des getters différents en fonction du niveau de visibilité
  - [ ] Utiliser les /@JsonFilter/
    - Permet de spécifier les champs à conserver/retirer directement en attachant des filtres à l'*ObjectMapper*
  - [ ] Utiliser les *MixIn*
    - Permet de définir des classes abstraites mappant les différents champs d'un objet que l'on souhaite sérialiser
    - On peut alors ajouter ou surcharger les annotations de la classe cible
    - Permet d'ajouter ou de retirer des champs
  - [ ] Cloner l'exercice et modifier son contenu avant de le sérialiser
    - Me paraît un peu lourd comme solution
  - [X] Modifier directement le JSON généré
    - On le fait déjà pour corriger le type des entités
    - On peut se contenter d'ajouter des méthodes supplémentaires pour filtrer les autres champs
- Corriger les tests unitaires
  - Tests unitaires de PLM
    - Le chemin vers les fichiers des missions était incorrect
    - Correction de la construction du chemin
  - Tests unitaires de webPLM
    - Pendant un moment, le champ /cells/ du JSON d'un *BuggleWorld* correspondait à une liste des cellules
      - Ceci permettait de réduire la taille du JSON
      - Plutôt que d'envoyer un tableau à 2 dimensions de *BuggleWorldCell* au format JSON, dont certaines étaient équivalentes à la cellule par défaut
      - On se contentait d'envoyer un tableau 1D contenant seulement les cellules étant différentes de la cellule par défaut
    - Depuis le passage à Jackson, on envoie de nouveau un tableau 2D comportant l'ensemble des cellules
    - Modification des tests pour correspondre à ce changement
- Trouver un moyen de récupérer la date de la dernière version du code de l'élève que l'on stocke
  - Les élèves semblent avoir trouvé par eux-mêmes la solution
*** Planned
**** CANCELLED Implémenter *ToJSON*
- [ ] Univers des tests unitaires
- [ ] Univers des tortues
...
- [ ] Univers du lander
**** CANCELLED Implémenter un constructeur à partir d'un *JSONObject*
- [ ] Univers des tests unitaires
- [ ] Univers des tortues
- [ ] Univers du drapeau hollandais
- [ ] Univers des pancakes
...
- [ ] Univers de light bot
- [ ] Univers du lander
**** DONE Ajouter la configuration Jackson pour les différents univers
- [X] Univers des tests unitaires
**** DONE Corriger les tests unitaires
- [X] Tests unitaires de PLM
  - /ExerciseFactory.computeMissions()/ semble poser problème
- [X] Tests unitaires de webPLM
  - L'erreur semble venir de la gestion des /cells/ dans /buggleworld.factory.spec.js/
**** DONE Configurer Jackson pour générer différents JSON en fonction de l'utilisation et du destinataire
- On souhaite envoyer le JSON d'un exercice à l'utilisateur
  - /defaultSourceFiles/ devrait être retiré
  - On a juste besoin de l'instruction dans la langue courante
- On souhaite envoyer le JSON d'un exercice au juge
  - On a juste besoin du /defaultSourceFile/ du langage courant
  - On a pas besoin des instructions
  - On a pas besoin des /steps/ des mondes solutions
- On souhaite stocker le JSON d'un exercice dans un fichier
  - On souhaite tout conserver
**** DONE Trouver un moyen de récupérer la date de la dernière version du code de l'élève que l'on stocke
- Si on veut gérer correctement tous les cas avec le localStorage, il faut comparer à la réception du code de serveur les dates de la version du serveur et de la version du localStorage
- Voir comment fournir la date de la version du serveur au client
** Semaine du <2016-05-02 lun.> au <2016-05-04 mer.>
*** Done
- Ajout d'une variable /Exercise.directory/
  - Permet de spécifier le répertoire contenant les exercices
  - Utiliser par les exercices lorsqu'ils souhaitent accéder à un fichier (.map notamment)
  - Initialiser à /exercises/ par défaut
- Ajouter la configuration Jackson pour les différents univers
  - Univers des tris
    - Suppression au passage des *Actions*, objets auparavant utilisés pour la vue du client lourd Java
*** Planned
**** DONE Ajouter la configuration Jackson pour les différents univers
- [X] Univers des tris
**** DONE Re-implémenter les exercices manquants
- [X] Maze
- [X] Sort.basic
** Semaine du <2016-05-09 lun.> au <2016-05-13 ven.>
*** Done 
- Re-implémenter les exercices manquants
  - Sort.dutchflag
    - Suppression du champ /entity/ dans *DutchFlagOperation*
  - Sort.baseball
    - Suppression du champ /entity/ dans *BaseballOperation*
    - Suppression de *BaseballMove*, on a juste besoin d'un compteur /moveCount/ dans *BaseballWorld*
  - Sort.pancakes
    - Suppression du champ /entity/ dans *PancakeOperation*
- Ajouter la configuration Jackson pour les différents univers
  - Univers des pancakes
    - Le JSON généré par Jackson permet bien de générer le modèle client
    - Par contre, la représentation du monde ne correspond pas au résultat attendu
      - Le monde solution est par exemple à l'envers
      - Appliquer les opérations solutions au monde initiale ne permet pas d'obtenir la réprésentation du monde solution
    - La liste des pancakes envoyée par le serveur est dans l'ordre inverse de l'ordre attendu par le côté client
      - Ceci est dû à cette ligne: https://github.com/BuggleInc/webPLM/blob/master/app/json/world/pancake/PancakeToJson.scala#L12
      - L'opération /+:/ ajoute en tête de liste le nouvel élément
    - Le modèle client pour l'univers des pancakes est donc implémenté de façon à fonctionner avec une liste inversée par rapport au modèle Java
    - Inversion de la liste des pancakes lorsqu'on la reçoit côté client pour pallier à ce problème
  - Univers des turmites
    - Le JSON généré est trop lourd (140Mo pour l'exercice *LangtonColors*, 100Mo si on désactive le /pretty print/)
      - On ne peut pas pusher les fichiers de plus de 100Mo sur GitHub
    - Modification de la config Jackson de *BuggleWorldCell*
      - Certains champs, tels que /color/, /hasBaggle/, /leftWall/..., conservent souvent leur valeur par défaut
      - On peut améliorer la sérialisation des *BuggleWorldCell* en ajoutant ces champs que si nécessaire (valeur modifiée)
      - Ajout de *BuggleWorldCellFilter*
    - A permis de réduire la taille du JSON (136Mo, 96Mo sans /pretty print/)
    - J'ai préféré désactiver des mondes dans cet exercice pour atteindre une taille plus raisonnable (65Mo avec /pretty print/)
*** Planned
**** DONE Ajouter la configuration Jackson pour les différents univers
- [X] Univers du drapeau hollandais
- [X] Univers du baseball
- [X] Univers des pancakes
- [X] Univers des tours de hanoï
- [X] Univers des turmites
**** DONE Re-implémenter les exercices manquants
- [X] Sort.dutchflag
- [X] Sort.baseball
- [X] Sort.pancake
- [X] Recursion.hanoi
- [X] Turmites
** Semaine du <2016-05-17 mar.> au <2016-05-20 ven.>
*** Done
- Ajout de la configuration Jackson pour les différents univers
  - Univers des listes récursives
    - Cet univers hérite de l'univers des tests unitaires
    - Sa configuration est donc déja en partie écrite
    - Seul l'affichage de la liste côté client rencontre un problème
      - Le JSON reçu présente la liste sous la forme suivante
        - { head : 1, tail : { ... } }
    - Il faut convertir ce JSON en liste d'entiers
    - La conversion peut s'effectuer
      - [X] Lors de la sérialisation, côté serveur, en utilisant un *CustomSerializer*
      - [ ] Lors de la désérialisation, côté client
    - Effectuer la conversion lors de la sérialisation me paraît une solution plus adaptée
      - Permet d'obtenir un JSON plus concis
      - Permet de ne pas avoir à faire de distinction entre *BatWorld* et *ConsWorld* côté client
    - Problème avec *CustomRecListSerializer*
      - Dès que *CustomReclistSerializer* est utilisé, une erreur silencieuse doit se déclencher car l'application ne répond pas
      - Même en remplaçant le code de *CustomReclistSerializer* (pourtant pas compliqué) par un simple ~gen.writeNumber(5);~, le serveur n'arrive pas à sérialiser des *RecList*
    - Bug lié au fait que les paramètres des *BatTest* soient typés en tant qu'*Object*?
*** Planned
**** DONE Ajouter la configuration Jackson pour les différents univers
- [X] Univers des tortues
**** DONE Re-implémenter les exercices manquants
- [X] Recursion.logo
** Semaine du <2016-05-23 lun.> au <2016-05-27 ven.>
*** Done
- Ajout de la configuration Jackson pour les différents univers
  - Univers des listes récursives
    - Puisque je n'arrive pas à faire fonctionner le *CustomSerializer* dans ce cas, changement d'approche
    - Lors de la sérialisation, ajout d'un champ /list/ correspondant à /RecList.toList()/
    - Ajout de /@JsonIgnore/ sur les champs /head/ et /tail/
    - Côté client, on se contente de vérifier lors de la construction d'un BatTest si le paramètre possède un champ /list/
    - Problème avec les entités en Scala
      - *scala.collection.immutable.List* possède aussi des méthodes /head()/ et /tail()/ pour parcourir la structure de donnée
      - Il a donc été décidé d'écrire les entités Scala en considérant que le code de l'élève reçoit en paramètre une *List* au lieu d'une *RecList*
      - La conversion *RecList* -> *List* s'effectuait automatiquement grâce à ce code https://github.com/BuggleInc/PLM/blob/javaUI/src/plm/core/model/lesson/ExerciseTemplatingEntity.java#L120-L133
      - Ce morceau de code a été supprimé au cours de refactoring
      - [X] Re-implémenter ce code
	- Sous quelle forme?
	- [X] Ajouter /toScalaList()/ à *RecList* ?
	- Pas d'autres idées pour le moment
      - [ ] Refactorer les entités Scala pour utiliser *RecList* dorénavant
    - Ajout de /toScalaList()/
      - Faire la conversion directement dans le code Java s'annonce difficile
      - Je préfère intéragir avec un objet Scala se chargeant de la conversion
      - Suivi du tuto: http://alvinalexander.com/scala/how-to-wrap-scala-traits-used-accessed-java-classes-methods
      - Pour faciliter la conversion en *List*, on ne stocke plus en paramètre une *RecList* mais un *int[]*
    - Permet de corriger le problème des entités Scala
    - Nouveau problème: les entités en Python
      - Vu que dorénavant on stocke des *int[]*, il faut générer une *RecList* à partir de ce tableau dans les entités Java et Python
      - En Java, on le fait naturellement dans l'entité
      - Ajout d'une entité *ConsEntity* qui ajoute automatiquement au script le code correspondant dans le cas du Python
** Semaine du <2016-05-30 lun.> au <2016-06-03 ven.>
*** Done
- Etudier *Open Roberta Lab*
  - Ils ont créé leur propre langage de programmation: *NEPO*
    - https://mp-devel.iais.fraunhofer.de/wiki/display/ORInfo/Programmieren+mit+NEPO
  - Besoin d'installer un OS custom pour que le robot puisse fonctionner avec *Open Roberta Lab*
    - https://mp-devel.iais.fraunhofer.de/wiki/display/ORInfo/EV3+vorbereiten
    - Pour générer l'image de l'OS, la documentation se trouve ici: https://github.com/OpenRoberta/robertalab/wiki/Creating-the-OR-leJOS-image
  - Architecture du projet
    - /EV3Menu/
      - J'ai encore du mal à voir de quoi il s'agit
      - Serait-ce le code de la surcouche de l'OS pour le robot?
      - Ou l'application permettant d'initer la connection au serveur depuis le robot
    - /JavaForLejos/
      - Contient juste ce qui a l'air d'être un test pour vérifier la bonne configuration du robot
    - /OpenRobertaParent/
      - A juste l'air de servir pour l'installation
    - /OpenRobertaRobot/
      - Convertit le code généré à l'aide de Blockly en code pour le robot
      - Contient aussi les objets permettant au serveur de communiquer avec le robot
    - /OpenRobertaRuntime/
      - On y retrouve de nouveau des objets permettant au serveur de communiquer avec le robot
      - Contient aussi l'objet *Hal* qui représente le robot
      - Tourne sur le robot?
	- Compilé pour ev3: https://github.com/OpenRoberta/robertalab/blob/master/OpenRobertaRuntime/crosscompiler-ev3-build.xml
    - /OpenRobertaServer/
      - Enfin de la doc \o/ : https://github.com/OpenRoberta/robertalab/tree/master/OpenRobertaServer
    - /OpenRobertaShared/
      - Contient les objets représentant les différents *Sensors* du robot et les *Actions* possibles
      - Ce sont ces objets qui sont utilisés par *Hal* pour manipuler le robot
    - /OpenRobertaUSB/
      - Contient les objets permettant d'établir une connection USB entre le robot et le serveur
      - Des infos sur le workflow ici: https://mp-devel.iais.fraunhofer.de/wiki/display/ORInfo/EV3+Roboter+und+USB+Verbindung
- Ajout de la configuration Jackson pour les différents univers
  - Univers des listes récursives
    - Découverte d'un autre problème: les *RecList* en résultat de la méthode de l'utilisateur en Scala
    - Il semblerait que le test d'égalité entre le résultat du monde solution et du monde utilisateur échoue si la liste est vide
      - La valeur *Nil* semble poser des problèmes à comparer
- Uniformiser TurtleArt
  - Auparavant, on avait qu'un exercice *TurtleArt* qu'on initialisait plusieurs fois avec des paramètres différents
  - Création d'exercices distincts pour chacun d'entre eux
*** Planned
**** DONE Etudier *Open Roberta Lab*
- *Open Roberta Lab* est un site offrant une interface pour développer à l'aide d'outils visuels pour des robots *EV3*
  - https://lab.open-roberta.org/
- Comprendre le projet et ses différents composants
  - Définition des blocs de programmation
  - Génération et compilation du code pour le robot
  - Mise en place de la connexion entre l'interface de programmation et le robot
**** DONE Uniformiser TurtleArt
...
**** DONE Re-implémenter les exercices manquants
- [X] TurtleArt
** Semaine du <2016-06-06 lun.> au <2016-06-10 ven.>
*** Done
- Ajout de la configuration Jackson pour les différents univers
  - Univers des listes récursives
    - Ajout de *ScalaHelper*, objet Scala mettant à disposition des méthodes pour comparer /Nil/ et /null/ ou convertir un objet de type *int[]* en *List[Int]*
    - Ajout de *RecListWrapper*, objet Scala permettant de comparer une *RecList* à une *List[Int]*
    - Grâce à ces helpers, j'ai pu corriger les méthodes de comparaison la fonction /winning()/ dans le cadre des *ConsExercise*
    - Dernière ombre au tableau: la sérialisation de *List[Int]* en JSON
      - Trouvé un projet mettant à disposition un module se chargeant de sérialiser ces objets: https://github.com/FasterXML/jackson-module-scala
    - Débuggage de l'interface graphique
      - Les tableaux n'étaient pas affichés correctement, les crochets étaient manquants
      - Modification de la vue pour gérer correctement les tableaux
- Re-implémenter les exercices manquants
  - Recursion.cons
    - Maintenant que l'univers est débuggé, je commence à ajouter les différents exercices de cette leçon
*** Planned
**** DONE Ajouter la configuration Jackson pour les différents univers
- [X] Univers des listes récursives
** Semaine du <2016-06-13 lun.> au <2016-06-17 ven.>
*** Done
- Résoudre le problème de synchronisation entre le repo local et le repo GitHub lors du 1er démarrage de webPLM
  - Le problème provient plus précisement de cette ligne : https://github.com/BuggleInc/PLM/blob/refactor-lessons-exercises/src/plm/core/model/tracking/GitUtils.java#L127
  - En supprimant ma copie locale du repo, j'ai retrouvé un temps de lancement correct
  - Ce qui est perturbant est que je ne pushais pas ma copie locale sur GitHub
  - Le repo distant était donc en retard d'un grand nombre de commits par rapport au repo local
  - Je n'avais aucune raison d'effectuer un merge des repos dans ce cas
  - Reste à voir comment éviter d'effectuer cette ligne lorsqu'aucun nouveau commit n'a été récupéré depuis GitHub
  - L'objet *FetchResult* retourné par la commande fetch possède une fonction /getTrackingRefUpdates()/
  - Cette fonction semble permettre de récupérer les références locales qui ont été mises à jour depuis le repo distant : http://download.eclipse.org/jgit/docs/jgit-2.0.0.201206130900-r/apidocs/org/eclipse/jgit/transport/OperationResult.html
  - S'assurer que *TrackingRefUpdate* correspond bien à ce que l'on souhaite
    - [X] Voir le résultat de /getTrackingRefUpdates()/ si la branche n'existe pas sur le repo distant
      - Une *GitException* est levée dans ce cas, on ne récupère donc pas de *FetchResult*
    - [X] Voir le résultat de /getTrackingRefUpdates()/ si la branche existe sur le repo distant et est en avance sur le repo local
      - La branche est listée dans la collection retournée par /getTrackingRefUpdates()/
    - [ ] Voir le résultat de /getTrackingRefUpdates()/ si le branche existe sur le repo distant mais est en retard sur le repo local
- Re-implémenter l'aide des exercices
  - Récupération de l'aide et de ses traductions
    - Se fait de la même façon que les consignes de l'exercice
    - *ExerciseFactory* récupère le type de monde présent dans l'exercice et accède à la fiche d'aide associée
    - Stocke les différentes traductions dans *Exercise*
    - Met à disposition pour récupérer une fiche de traduction en fonction de sa langue et du langage de programmation choisi
  - Renommage du champ correspondant côté client
    - /api/ -> /help/
    - Permet de correspondre au bouton affichant l'aide 
*** Planned
**** DONE Ajouter la configuration Jackson pour les différents univers
- [X] Univers des listes récursives
**** DONE Re-implémenter les exercices manquants
- [X] Recursion.cons
**** DONE Passer à une librairie prévue pour la sérialisation/désérialisation au format JSON
- J'ai commencé à implémenter des méthodes d'import/export au format JSON des différents objets formant les univers à l'aide de /JSON.Simple/
- Mais on rencontre des difficultés avec certains cas particuliers
  - Dans /Exercise.toJSON()/, il faut spécifier le type des mondes composant cet exercice
  - Dans /World.toJSON()/, il faut spécifier le type des entités peuplant ce monde
    - Dans le cas des mondes solutions, puisqu'on a executé la démo, le type obtenu est de la forme /plm.runtime1.nomEntité/ car l'entité a été généré dynamiquement
    - Il faut donc remplacer ce type par le type des entités du monde initial
  - Dans /BatTest.toJSON()/, il faut spécifier le type des paramètres de ce test
- Les solutions apportées ne sont pas très élégantes et consistent plus en des hacks qu'autre chose
- Est-ce que remplacer /JSON.Simple/ par une librairie telle que [[https://github.com/FasterXML/jackson][Jackson]] ou [[https://github.com/google/gson][Gson]] permettrait d'accéder à des fonctionnalités apportant une meilleure solution à ces problèmes?
- Vu que nous utilisons les mêmes méthodes pour générer le JSON envoyé au client et aux juges, il faudrait aussi pouvoir modifier facilement le JSON généré pour filtrer des champs
  - On a besoin d'envoyer le *SourceFile* aux juges pour qu'ils puissent correctement générer l'entité de l'élève et l'executé
  - Mais on ne veut pas le transmettre au client puisqu'il contient le code de la correction
**** DONE Re-implémenter l'aide des exercices
- Pour chaque type de monde, une fiche d'aide avec les commandes utilisable est disponible.
- Il n'y a plus de moyen d'afficher cette fiche depuis le refactoring
- Re-implémenter cette fonctionnalité
** Semaine du <2016-06-27 lun.> au <2016-07-01 ven.>
*** Done
- Résoudre le problème de synchronisation entre le repo local et le repo GitHub lors du 1er démarrage de webPLM
  - [X] Voir le résultat de /getTrackingRefUpdates()/ si le branche existe sur le repo distant mais est en retard sur le repo local
    - Aucune branche n'est listée dans la collection retournée par /getTrackingRefUpdates()/
- Déployer une version de test de la nouvelle version de PLM
  - Génération des nouvelles images Docker de
    - webPLM
      - Erreur lors du lancement de l'image
      - Une erreur d'encodage se produit lors de la lecture des fichiers des leçons
      - L'erreur ne se produisait pas dans mon environnement de développement puisque la locale définie est en UTF-8
      - Dans l'image Docker debian récupérée, aucune locale semble n'être définie
      - Modification du code pour spécifier l'encodage du fichier : UTF-8
    - plm-judge
      - Modification du Dockerfile pour se contenter d'ajouter le jar résultant de /sbt assembly/ et de le lancer
- Visio avec les membres de l'université de Savoie et de Pentila
  - Présentation de l'existant, notamment de l'architecture actuelle
  - A permis de répondre aux questions qu'ils se posaient après avoir jeté un oeil à PLM
- Rebase des branches de /refactoring/
  - Pour que le merge avec la branche /master/ se passe correctement, j'ai effectué un rebase au préalable pour intégrer les derniers commits
  - Correction des conflits et des bugs liés au rebase
- Gérer les images des leçons
  - En remplaçant la route par un appel à un *Controller*, j'ai pu vérifier que la route est correctement appelée
  - Reste à voir pourquoi les fichiers ne sont pas correctement servis
*** Planned
**** DONE Résoudre le problème de synchronisation entre le repo local et le repo GitHub lors du 1er démarrage de webPLM
- Lorsqu'on démarre le serveur, celui-ci tente de synchroniser le repo local correspondant à la session de l'utilisateur par rapport à celui sur GitHub
- J'ai remarqué que la synchronisation prend un temps considérable lors du 1er démarrage du serveur de la journée
- Ceci s'avère être bloquant pour l'utilisateur puisqu'on essaie d'accéder à sa session lorsqu'il tente d'accéder à un exercice
  - pour enregistrer cet évènement
  - pour récupèrer sa dernière version du code
- L'acteur gérant la session de l'utilisateur ne répond pas tant qu'il a pas fini de synchroniser les repos
- Trouver l'origine de ce ralentissement/blocage
** Semaine du <2016-07-04 lun.> au <2016-07-08 ven.>
*** Done
- Correction d'erreurs de code mineures
  - Suppression de ; en trop
  - Suppression de /import/ inutiles
*** Planned
** Semaine du <2016-07-11 lun.> au <2016-07-15 ven.>
*** Done
- Gérer les images des leçons
  - Plutôt que d'utiliser le répertoire /dist/lessons/ pour conserver les leçons, utilise le répertoire /lessons/
  - Ajout du répertoire /lessons/ dans le build
    - En tant que ressources : unmanagedResourceDirectories in Compile += baseDirectory.value / "lessons"
    - Et en tant qu'assets : unmanagedResourceDirectories in Assets += baseDirectory.value / "lessons"
  - Les fichiers sont dupliqués lors du build, mais cela permet d'avoir un comportement plus simple
    - Pour accéder aux fichiers dans le code
    - Pour fournir les fichiers aux clients
  - Ajout des images manquantes
  - Ajout de la route pour accéder aux fichiers des leçons
    - Accès limité aux images au format PNG
- Gérer les images présentes dans les consignes d'exercices
  - Même raisonnement que pour les images des leçons
  - Ajout du répertoire /exercises/ dans le build
    - En tant que sources : unmanagedSourceDirectories in Compile += baseDirectory.value / "exercises"
    - En tant que ressources : unmanagedResourceDirectories in Compile += baseDirectory.value / "exercises"
      - Permet d'accéder aux fichiers .py, .blocky, .json
    - En tant qu'assets : unmanagedSourceDirectories in Assets += baseDirectory.value / "exercises"
  - Modification du code de *ExerciseActor* pour utiliser le système de ressources plutôt que les fichiers directement
  - Ajout de la route pour accéder aux images des exercices
    - Accès limité aux images au format PNG et JPEG
- Gérer les exercices manquants
  - Modification de /ExercisesActor.getExercise()/ pour renvoyer une *Option[Exercise]*
  - Permet de gérer le cas où l'exercice n'a pas été trouvé
  - Envoi du message /exerciseNotFound/ le cas échéant
  - Correction du JS pour rediriger correctement l'utilisateur
    - Le service /$state/ n'était pas injecté dans *ExerciseController*
- Re-implémenter le passage à l'exercice suivant
  - L'ID de l'exercice diffère entre la classe *Lesson* et la classe *Exercise*
    - Dans *Lesson*, on utilise des IDs de la forme /environment.Environment/
    - Tandis que dans *Exercise*, on utilise l'ID /Environment/
  - Le client n'arrive donc pas à comparer l'ID de l'exercice courant aux IDs de la leçon
  - Correction du constructeur de *ExerciseTemplated* pour utiliser la classe en tant qu'ID
  - Modification de *ExerciseRunner* pour récupérer le bon nom de l'entité
    - Auparavant on utilisait l'ID de l'exercice directement
  - Modification du code client pour mettre à jour l'ID de l'exercice courant lorsqu'on reçoit le JSON de ce dernier
  - Mise à jour du JSON des exercices
- Désactivation de la leçon /bat.string1/
  - Les exercices de cette leçon n'étant pas encore implémentés, je préfère la désactiver
- Re-implémenter l'évènement Git /reverted/
  - Ajout du message /RevertExercise/ dans *GitActor*
    - Supprime les fichiers de l'exercice pour le langage de programmation courant
    - Génère le message de commit correspondant
      - De la forme : /{"kind": "reverted", "exo":"traversal.snake.Snake","lang":"Scala"}/
    - Commit la suppression
  - Ajout du message /reset/ du serveur au client pour réinitialiser le contenu de l'IDE
- Amélioration de l'UI
  - Mes collègues ont pu faire un nouveau retour sur l'UI lors des tests effectués sur la nouvelle version
  - Modification du CSS
    - de la consigne pour éviter que la barre de scroll ne déborde sur le texte
    - de la page d'exercice pour réduire la taille des marges
    - de la page d'exercice pour réduire la taille de la séparation entre les blocs de gauche et de droite
*** Planned
**** DONE Gérer les images des leçons
- Les images des leçons ne s'affichent pas correctement
- La route qui permet de servir ces images n'a pas l'air de fonctionner
- Corriger cela
**** DONE Gérer les images présentes dans les consignes d'exercices
- Jusqu'à maintenant, on pouvait accéder à ces images via leur path dans le JAR de PLM
- Dorénavant, ces images se trouvent dans le répertoire source /exercises/
- Rendre accessible ces images via une URL
- Modifier les leçons pour mettre à jour l'URL des images
**** DONE Gérer les exercices manquants
- Lorsque l'utilisateur essaie d'accéder à un exercice manquant, une *NullPointerException* est déclenchée
- Vérifier que l'exercice existe avant d'essayer de le cloner
- Prévenir le client que l'exercice est manquant
- Rediriger l'utilisateur vers un autre exercice
**** DONE Re-implémenter le passage à l'exercice suivant
- Lorsqu'on réussit un exercice, un message nous informe de notre succès et nous propose de passer à l'exo suivant
- Ceci ne fonctionne plus
- Corriger ce problème
**** DONE Re-implémenter l'évènement Git /reverted/
** Semaine du <2016-07-18 lun.> au <2016-07-22 ven.>
*** Done
- Afficher un badge pour indiquer les exercices réussis
  - Avant d'envoyer la liste des exercices d'une leçon à un utilisateur, vérifie quels exercices ont été réussis et avec quel langage de programmation
    - Ajout de méthodes dans *SessionActor* et *GitActor* pour vérifier si un exercice a été réussi dans un langage de programmation donné
    - Dans /Lecture.toJSON()/, aggrège les résultats et l'ajoute au JSON généré
  - Côté client, ajout d'une nouvelle directive dans la liste des exercices
  - Nouvelle ligne composée des logos des différents langages de programmation supportés par l'exercice
    - Si l'exercice n'a pas encore été réussi dans un langage de programmation donné, le logo apparaît en noir et blanc
    - Sinon, le logo apparaît normalement
- Mettre à jour les fichiers Dockerfile
  - Pour *webPLM* et *PLM-judge*, on retrouve les mêmes problèmatiques qu'auparavant
  - Pour avoir un build d'image Docker simple et reproductible, il faudrait que celui-ci:
    - Installe les dépendances tels que Java, Scala, git, activator/sbt
    - Clone le projet
    - Build le projet
    - Et se prépare à exécuter le binaire de l'application
  - Sauf que activator/sbt nécessite beaucoup de dépendances pour fonctionner
    - Ces dépendances se trouvent sur des repos Maven
    - Les télécharger prend du temps (5 à 10min lors de mon dernier test)
    - Et il semblerait que si on construit consécutivement plusieurs images, les repos Maven commencent à rejeter nos requêtes pour télécharger les dépendances
    - Bien entendu, la construction de l'image échoue si une dépendance est manquante
  - Ceci nuit au caractère reproductible du build et s'avère plutôt lent
  - Les dépendances de SBT téléchargées sont mises en cache dans le répertoire ~/.ivy2
    - Actuellement, ce répertoire est éphémère et est regenéré à chaque build
  - Utiliser un volume partagé pour ce répertoire permettrait de limiter les interactions avec les repos Maven
    - Ceci améliorerait la rapidité et la reproductibilité du build
  - Mais Docker ne supporte pas le partage de volume durant le build
  - [ ] Décomposer le build en 2 images Docker ?
    - La 1ère permettrait de compiler l'application
      - Mettrait en place l'environnement pour la compilation
      - CMD: compiler l'application
      - Utiliserait plusieurs volumes partagés
        - Les sources du projet
        - Le répertoire ~/ivy2 pour conserver le cache des dépendances d'un build à l'autre
        - Le répertoire cible qui contiendrait le binaire généré
    - La 2nde permettrait d'exécuter l'application
      - Mettrait en place l'environnement pour l'exécution
      - Récupérerait le binaire de l'application
      - CMD: exécuter l'application
  - L'image pour compiler un projet sbt existe déjà
    - https://hub.docker.com/r/1science/sbt/
    - On peut donc compiler facilement et de façon reproductible *webPLM* et *PLM-judge* à partir des sources
      - docker run -ti --rm -v /path/to/webPLM:/app -v "$HOME/.ivy2":/root/.ivy2 1science/sbt sbt clean stage
      - docker run -ti --rm -v /path/to/PLM-judge:/app -v "$HOME/.ivy2":/root/.ivy2 1science/sbt sbt assembly
    - Mais je préfère avoir directement les sources dans l'image, de façon à pouvoir rebuilder exactement le même binaire aisément
- Mettre à jour les fichiers docker-compose.yml
  - Problème avec les links entre containers
    - Dans l'ancienne version de docker-compose, on pouvait utiliser la commande /links/ pour permettre aux containers de communiquer entre eux
    - Cette commande générait des variables d'environnement de la forme NOM_SERVICE_ADDR et NOM_SERVICE_PORT
    - *webPLM* et *PLM-judge* sont configurés pour utiliser ces variables d'environnement lorsqu'ils souhaitent interagir avec un autre composant
    - Pour pouvoir fonctionner en dehors de /docker-compose/, des valeurs par défaut sont utilisées si ces variables d'environnement sont indéfinies
    - Dans la nouvelle version, la commande /links/ n'utilise plus de variables d'environnement mais directement des hostnames
    - Par exemple /links : - db/ permet de communiquer avec le service /db/ via l'URL http://db
    - Par contre, le code reposant sur les variables d'environnement est donc obsolète
    - Mais comment faire un fallback vers une URL par défaut de façon élégante avec ce nouveau système de links?
    - [ ] Générer une variable d'environnement dans /docker-compose/ et vérifier son existence dans le code?
*** Planned
**** DONE Afficher un badge pour indiquer les exercices réussis
- Actuellement, rien n'indique à l'utilisateur quels exercices il a déjà réussi
- Ajouter des badges sur les exercices réussis en fonction des langages de programmation choisis
** Semaine du <2016-07-25 lun.> au <2016-07-27 mer.>
*** Done
- Mettre à jour les fichiers Dockerfile
  - Ajout d'un Dockerfile permettant de générer une image servant à builder une application Play
    - Cette image peut être utilisée dans le workflow de build pour webPLM ou PLM-judge
  - Mise à jour du Dockerfile pour webPLM
    - Maintenant on se contente de copier les binaires de l'environnement de l'utilisateur dans l'image docker
    - Ajout d'un script pour simplifier le workflow de génération de l'image
    - Ce script build webPLM à l'aide de l'image docker Play puis génère l'image docker de webPLM
    - Mise à jour du README pour refléter ces changements
  - Mise à jour du Dockerfile pour PLM-judge
    - Modification du Dockerfile pour utiliser le JAR de PLM-judge
    - Adapatation du script utilisé pour build webPLM pour PLM-judge
    - Ajout d'un README pour expliquer le nouveau fonctionnement
  - Modification des scripts pour pouvoir build le projet à partir de binaires
    - Au cours du développement, on souhaite parfois tester le nouveau comportement de l'application au sein de l'environnement Docker
    - Mais on souhaite éviter de commit/pusher alors qu'on est incertain du résultat
    - Ajout d'une option --bin
      - Indique aux scripts où récupérer les binaires du projet
      - Copie les binaires dans le répertoire target/
      - Passe directement à la construction de l'image finale
      - Exemple : ~./make.sh --bin ~/ADT-PLM/PLM-judge/target/ -v 2.0-rc1~
- Mettre à jour les fichiers docker-compose.yml
  - Définition de plusieurs docker-compose.yml correspondant à plusieurs cas d'utilisation
  - [ ] main-server
    - Permet de déployer les composants principaux sur le serveur principal
      - webPLM
      - plm-accounts
      - plm-profiles
      - rabbitmq
      ...
      - nginx
  - [ ] judge
    - Permet de déployer des juges sur les machines secondaires
    - Déploie
      - PLM-judge
  - [X] dev
    - Permet de déployer l'ensemble des composants nécessaires pour l'exécution
    - Permet de tester ces composants avant de déployer en prod
    - Déploie
      - webPLM
      - PLM-judge
      - rabbitmq
- Corriger la configuration de RabbitMQ
  - Modification du code pour ACK explicitement les différents messages
    - Le bug a été reproduit
  - J'ai re-essayé en démarrant les différents composants en dehors de Docker
    - Je n'ai pas réussi à reproduire ce bug
  - J'ai re-essayé en retirant l'option /restart/ du docker-compose sur les juges
    - Le bug a été reproduit
  - Il s'agit en fait uniquement d'un "bug" d'affichage
    - Lorsque un conteneur est redémarré, ses logs précédents sont de nouveaux ajoutés aux logs affichés par /docker-compose logs/
*** Planned
**** DONE Mettre à jour les fichiers Dockerfile
- Docker a évolué depuis qu'on a rédigé les fichiers Dockerfile
- Certaines commandes sont apparues et sont à privilégiées par rapport aux anciennes
  - COPY au lieu de ADD
- De même, le build de certains composants de l'application a évolué
  - On utilise maintenant /sbt assembly/ pour générer un JAR de PLM-judge
**** DONE Corriger la configuration de RabbitMQ
- En déployant les nouvelles images docker pour tester, j'ai pu remarqué un comportement étrange au niveau de la communication via RabbitMQ
- Au démarrage, le conteneur contenant le juge se connecte à la queue de message et attend une requête d'exécution
- Une fois que le juge a traité une requête, il s'auto-détruit
- Le conteneur redémarre alors
- Sauf que lorsque le nouveau judge se connecte à la queue de message, il reçoit les messages précédents
- Pour chaque message, il le traite et s'auto-détruit
- 2 points particulièrement étranges :
  - Le conteneur Docker n'a pas l'air de rédémarrer lorsque le juge traite les messages précédents, alors que le juge semble bien s'auto-détruire à chaque message
  - Comment le juge arrive à rejouer l'ensemble des messages précédents s'il s'auto-détruit après chaque exécution ?
    - Ca me semblerait plus logique qu'il tombe dans une boucle infinie et rejoue le 1er message à chaque fois
** Semaine du <2016-08-01 lun.> au <2016-08-05 ven.>
*** Done
- Mettre à jour les fichiers docker-compose.yml
  - Ajout du docker-compose pour déployer les juges
  - Ajout du docker-compose pour déployer les services sur la machine principale
- Déployer la nouvelle version
  - Mise à jour de docker engine et de docker-compose de la machine de TN
  - Génération des nouvelles images sur le serveur de TN
    - plm-nginx
    - webplm:2.0-rc2
    - judge:2.0-rc1
  - Les scripts fonctionnent correctement
  - Problème avec PLM-accounts et PLM-profiles
    - L'URL de connexion à la base de données n'est plus valide
      - On utilisait la variable d'environnement DB_PORT_27017_TCP_ADDR générée automatiquement par docker-compose
      - Maintenant c'est la variable d'environnement DB_ADDR
    - Modification du code source pour corriger l'URL
    - La re-génération des images échoue
      - Des dépendances sont dépréciées
  - Modification des images existantes de PLM-accounts et PLM-profiles
    - Comme la modification à effectuer ne concerne dans les 2 cas qu'une seule ligne, il était plus simple pour le moment de modifier les images existantes
    - Workflow de mise à jour
      - Démarrer un conteneur en mode interactif basé sur l'image à mettre à jour
	- /docker run -it plm-accounts /bin/bash/
      - Modifier la ligne concernée
      - Commit la modification
	- /docker commit <id du container> plm-accounts:temp/
    - Rencontre d'un problème concernant la CMD de la nouvelle image
      - Démarrer en mode interactif le conteneur et commit ses modifications à changer la CMD lancée au démarrage du conteneur
	- /grunt/ -> /bin/bash/
      - J'ai dû refaire l'étape précédente en modifiant la commande utilisée pour démarrer le conteneur et commit la modification
	- /docker run -it plm-accounts-temp grunt/
	- /docker commit <id du container> plm-accounts:1.1/
- Configurer *New Relic*
  - [X] Ajouter un agent pour obtenir des informations plus détaillés sur les applications
    - Un agent est disponible pour les applications Java
      - Doc dispo ici : https://docs.newrelic.com/docs/agents/java-agent/frameworks/play-installation-java
    - Il faudrait donc modifier les images *Docker* pour l'utiliser
      - Quelles sont les bonnes pratiques dans ce cas de figure?
	- Générer plusieurs images *Docker* : des images standards et d'autres avec *New Relic* ?
    - Création de nouvelles images *Docker* pour webPLM et PLM-judge pour utiliser l'agent de *New Relic*
  - [X] Remonter les ressources utilisées par la machine
    - Permet de remonter
      - CPU
      - RAM
      - Disk I/O
      - Network I/O
    - Installation d'un service de *New Relic* pour remonter ces données pour la machine de TN
      - Doc dispo ici : https://docs.newrelic.com/docs/servers/new-relic-servers-linux/getting-started/new-relic-servers-linux
      - Permet de voir ces stats par process
    - Configuration de ce service pour fonctionner avec *Docker*
      - Doc dispo ici : https://docs.newrelic.com/docs/servers/new-relic-servers-linux/getting-started/new-relic-servers-docker
      - Permet d'avoir maintenant les stats par images *Docker*
  - [X] Ajouter une alerte en cas de stress de la machine
    - Installer le service de *New Relic* pour monitorer une machine Linux configure automatiquement une alerte par e-mail
    - Celle-ci est envoyé si une condition suivante est remplie
      - CPU > 80%
      - RAM > 90%
      - Disk IO > 90%
      - Espace de stockage rempli > 90%
    - Envoie aussi une alerte si le serveur arrête de report des données pendant plus de 5min
- Ajouter un texte alternatif aux images utilisés dans webPLM
  - Ajout du texte alternatif
  - Modification au passage du style pour espacer les boutons
- Ajouter les options de Docker pour limiter les ressources allouées aux containers
  - Ajout de l'option /cpu_share/ sur les différents services du serveur principal
    - Permet de donner un poids aux différents services pour l'accès aux ressources de la machine
  - Ajout de l'option /mem_limit/ et /memswap_limit/ sur les juges
    - Permet de limiter la quantité de RAM et la quantité de RAM + swap utilisable par un juge
- Débugger la leçon sur les turmites
  - L'erreur provient de la sérialisation puis désérialisation des exercices
  - L'attribut *World.parameters* n'est pas sérialisé correctement
  - Dans le cas des turmites, le paramètre /rule/ qui est de type *char[]* est sérialisé en tant que *String*
  - Ajout du type du paramètre dans la sérialisation
  - Regénération des sérialisations JSON
  - Erreur maintenant à cause du type *Color*
    - Dans certains exercices, notamment /methods.slug.SlugSnail/, une instance de *Color* est utilisé en tant que paramètre
    - Je n'arrive pas à configurer /Jackson/ pour utiliser mes sérialiseurs customs s'il rencontre une instance de leur type
    - Il utilise alors son sérialiseur par défaut
    - La désérialisation échoue ensuite car, le type étant correctement renseigné dans la sérialisation, on utilise mon désérialiseur custom
  - Modification de *CustomColorDeserializer* pour supporter les champs sérialiser avec le comportement par défaut de /Jackson/
- Ajouter la configuration Jackson pour les différents univers
  - Ces univers n'étant pas encore supportés par *webPLM*, il faudrait plutôt porter ces univers
- Résoudre le problème d'incrémentation du compteur avec Kamon
  - N'ayant reçu aucune réponse à ma question, je ne pense pas que *Kamon* soit adapté à notre cas d'utiliation
  - Il faudrait plutôt voir pour utiliser *New Relic* pour monitorer le workflow d'exécution
- Retravailler le workflow de déploiement des dockers
  - La nouvelle organisation du repo [[https://github.com/buggleinc/plm-dockers/tree/update][plm-dockers]] corrige ce problème
- Profiler l'application
  - *New Relic* nous permet d'observer la consommation en ressources des différents composants de l'application
- Corriger les tests d'intégration & Découper les tests d'intégration des leçons
  - Les leçons et exercices ayant été retirés de *PLM*, les tests d'intégration associés l'ont été aussi
  - Fusion de ces tâches avec la tâche /Re-implémenter les tests d'intégrations/
- Utiliser l'injection de dépendance
  - Le refactoring de *webPLM* et *PLM* a permis
    - De revoir l'architecture de l'application pour limiter le nombre de dépendances des composants
    - D'utiliser l'injection de dépendance là où c'était possible
*** Planned
**** DONE Mettre à jour les fichiers docker-compose.yml
- De même que pour Docker, docker-compose a évolué depuis l'an dernier
- La syntaxe des fichiers docker-composes.yml actuels est dépréciée
  - Pour le moment, l'ancienne syntaxe est toujours supportée pour une raison de rétro-compatibilité
- On est pas à l'abri d'une dépréciation totale de l'ancienne syntaxe
**** DONE Déployer la nouvelle version
- Maintenant que les fichiers Dockerfile et docker-compose.yml sont prêts, il est temps de mettre à jour la version de prod
**** DONE Ajouter un texte alternatif aux images utilisés dans webPLM
- Semblerait que les logos utilisés pour l'interface de login rencontrent des difficultés à s'afficher
- Ajouter un texte alternatif à ces images
**** DONE Ajouter les options de Docker pour limiter les ressources allouées aux containers
**** DONE Configurer *New Relic*
- *New Relic* est un service de supervision/monitoring
- Met à disposition des agents pour analyser le comportement des applications lancées
- Met aussi à disposition des services permettant de remonter les ressources utilisées par la machine
- Il est possible de configurer des alertes pour être informé d'un éventuel plantage du serveur
**** DONE Débugger la leçon sur les turmites
- Une erreur se produit à l'exécution du code de l'élève
  - java.lang.ClassCastException : java.lang.String cannot be cast to [C 
    Merci de corriger votre code.
    at plm.runtime1.LangtonColors.run (plm.runtime1.LangtonColors:17)
    at plm.core.lang.JVMCompiledLang.runEntity (null:-1)
    at plm.core.model.lesson.ExerciseRunner$3.run (null:-1) at java.lang.Thread.run (Thread.java:745)
- Corriger ce bug
**** DONE Publier les images Docker utilisées
- Cela permettra de déployer les juges aisément sur une nouvelle machine
- Cela permettra aussi à un nouvel utilisateur de déployer chez lui l'environnement aisément
- Mettre à jour les fichiers docker-compose.yml en concordance
**** DONE Retravailler le workflow de déploiement des dockers
...
**** CANCELLED Ajouter la configuration Jackson pour les différents univers
- [ ] Univers de light bot
- [ ] Univers du lander
**** CANCELLED Résoudre le problème d'incrémentation du compteur avec Kamon
**** CANCELLED Corriger les tests d'intégration
...
**** CANCELLED Découper les tests d'intégration des leçons
...
** Semaine du <2016-08-08 lun.> au <2016-08-12 ven.>
*** Done
- Problème avec l'agent Java de *New Relic* dans les juges
  - L'agent pose des problèmes de performances
  - Les juges mettent trop de temps à répondre
  - L'exécution est donc abandonnée
  - Utilisation de l'image /buggleinc/plm-judge:2.O-rc2/ à la place
- Ecrire les README pour les docker-compose.yml
  - [X] Ecriture du README pour l'environnement de dev
  - [X] Ecriture du README pour déployer les juges
  - [X] Ecriture du README pour déployer les composants du serveur principal
- Réaliser un script pour démarrer les juges sur les machines des étudiants
  - Ajout d'un script Bash dans *PLM-judge*
    - Si le JAR de *PLM-judge* n'est pas déjà présent, le télécharge
    - Set les variables d'environnement nécessaires pour se connecter à la queue de message
    - Démarre le juge puis le rédémarre tant qu'il n'a pas observé 5 erreurs
      - On parle d'erreurs propre au juge, et non pas d'erreurs liés au code de l'élève
      - L'erreur principalement attendue est l'échec de la connexion à la queue de message
  - Release d'archives /zip/ et /tar.gz/ contenant ce script
    - Ces archives permettent de conserver les permissions sur le script
    - Les élèves peuvent donc lancer le script sans utiliser de /chmod u+x launcher.sh/ au préalable
- Corriger le build des images Docker de PLM-accounts et PLM-profiles
  - Besoin de mettre à jour MEAN.JS
  - Pour ça, il faut fusionner l'historique du repo MEAN.JS
    - /git remote add mean git@github.com:meanjs/mean.git/
    - /git pull -X theirs --allow-unrelated-histories mean 0.4.x/
  - Problème avec l'authentification
    - MEAN.JS propose d'emblée un module pour gérer les utilisateurs
    - C'est celui qui est utilisé dans la version actuelle de PLM-accounts
    - Cependant, la fonction utilisée pour hasher a été modifiée
      - Auparavant : /crypto.pbkdf2Sync(password, this.salt, 10000, 64).toString('base64');/
      - Maintenant : /crypto.pbkdf2Sync(password, new Buffer(this.salt, 'base64'), 10000, 64).toString('base64');/
    - Mais celle-ci ne renvoie pas le même résultat pour un même couple (mot de passe, salt)
    - Ce changement est justifié ici: https://github.com/meanjs/mean/commit/08f1750d9450df834a0cf19d303e3c2f9e5da565#diff-240057bbb9109acf3b1745506df4b781
    - Voir ce que l'on peut faire pour les comptes existants?
      - [-] Se contenter de supporter les 2 méthodes de hash
	- [-] Déjà checker avec l'ancienne méthode, et avec la seconde si la 1ère échoue
	- [-] Ou ajouter un champ permettant de différencier les anciens utilisateurs des nouveaux et ainsi déterminer quelle méthode de hash utiliser
      - [-] Supporter les 2 méthodes de hash et regénérer le mot de passe d'un utilisateur lorsque celui-ci se connecte avec l'ancienne
      - [-] Faire une composition des 2 méthodes de hash
        - Au lieu de hash uniquement avec la nouvelle méthode les mots de passe, on peut hasher les mots de passes avec les 2 méthodes successivement
        - Il faudrait lancer un script au préalable pour hasher les mots de passe existants avec la nouvelle méthode
        - Mais comme le problème provient de comment le /salt/ est stocké, je n'ai pas l'impression que ça résoudrait le problème
    - Je rencontre un autre problème lié semblerait à la version de Node
      - La fonction /crypto.pbkdf2Sync(password, this.salt, 10000, 64).toString('base64');/ ne renvoie pas le même résultat en v0.12.14 qu'en v6.3.1
      - Je me mets en v0.12.14 de Node -> je créé un compte -> je peux me connecter -> je passe en v4.x -> je peux me connecter -> je passe en v6.x -> je peux plus me connecter
      - Je me mets en v6.x de Node -> je créé un compte -> je peux me connecter -> je passe en v4.x -> je peux plus me connecter
      - Par contre, si je démarre la console Node dans le terminal, peu importe la version de Node, le même appel à /crypto.pbkdf2Sync("password", "salt", 10000, 64).toString('base64');/ génère toujours le même résultat
    - En fait, les 2 problèmes sont liés
      - Avant le commit linké précédemment, le /salt/ stocké était brouillé
        - Du genre : �\u000b���<�\n\u001f\u00056\u00122yF�
      - Pour le même appel /crypto.pbkdf2Sync("password", "�\u000b���<�\n\u001f\u00056\u00122yF�", 10000, 64).toString('base64');/
        - Node v0.12.x | v4.x renvoie /ngxGurwC5e1Skko4D9w8b82Z6A/MpH5imCDIQiyyfhFkjYzkPSgepiFtxVHPAJ++7OsZf1PjhTxxrz1f2mnbiQ==/
        - Node v6.x renvoie /jbEXjC6BcWkHuYC9nrNwdnC2vUq3uY7MRagwekldb3qqB9V1eEWSCsFHLzqX8i1Q0ajQIbhwQH/kGZR1OdHGxA==/
      - Par contre, pour un /salt/ bien construit, l'appel à /crypto.pbkd2Sync("password", "salt", 10000, 64)/ renvoie le même résultat peu importe la version
      - On retrouve le problème précédent si on ajoute un accent au mot de passe
        - Utiliser d'autres caractères spéciaux tels que /?/ ou /#/ n'a pas l'air de reproduire le problème
      - Pour le même appel /crypto.pbkdf2Sync("pàssword", "salt", 10000, 64).toString('base64');/
        - Node v0.12.x | v4.x renvoie /G7bGPWNZPC0bEDoraY9nxzV/RlwBWV7JKE0hu+b86piEy2ZHqT+n0k409RAp5yy4CtktYce3lC9Y9UCuO8XQxA==/
        - Node v6.x renvoie /3KFr2KaeZVhpKcJF262k6+5VxpHfaVCWYXl69TBapIL+RsWCyyrbfXvKxc2AQLnC2yC7AtMFO8PXzBnH83fVVw==/
      - Ce problème est dû à l'encodage par défaut des *Buffer* dans Node
        - Node v0.12.x | v4.x, /crypto.pbkdf2Sync("pàssword", "salt", 10000, 64).toString('base64') === crypto.pbkdf2Sync(new Buffer("pàssword", "ascii"), "salt", 10000, 64).toString('base64')/
        - Node v6.x, /crypto.pbkdf2Sync("pàssword", "salt", 10000, 64).toString('base64') === crypto.pbkdf2Sync(new Buffer("pàssword", "utf8"), "salt", 10000, 64).toString('base64')/
    - Du coup, il faudra voir ce que l'on fait des comptes existants
      - [ ] Réinitialisation du /password/ et du /salt/ de chaque utilisateur après avoir migré à la nouvelle version de MEAN.JS
        - Faire un script pour générer un nouveau /password/ et /salt/ pour chaque utilisateur et mise à jour de la BDD
        - Envoi de mails pour avertir les utilisateurs et les inviter à changer leur mot de passe
      - [X] Supporter les 2 méthodes de hash et regénérer le mot de passe d'un utilisateur lorsque celui-ci se connecte avec l'ancienne
    - Modification du module /users/ pour
      - [X] Vérifier si l'utilisateur courant fait partie des anciens comptes ou des nouveaux
      - [X] L'authentifier avec la méthode de hash correspondante
      - [X] Générer un nouveau sel et mettre à jour le hash de son mot de passe s'il s'agit d'un ancien compte
*** Planned
**** DONE Ecrire les README pour les docker-compose.yml
- On possède maintenant plusieurs /docker-compose.yml/ permettant de déployer l'application ou une sous-partie
- Certains nécessitent de déclarer des variables d'environnement ou de monter des fichiers de configuration
- Rédiger des README pour expliquer le but de chacun de ces /docker-compose.yml/ et comment les utiliser 
**** DONE Réaliser un script pour démarrer les juges sur les machines des étudiants
- Plutôt que de télécharger et démarrer une instance de webPLM sur les machines des étudiants en cas de surcharge serveur, il peut être utile de juste démarrer un juge sur leur machine
- Réaliser un script
  - Récupérant le JAR de l'appli
  - Lançant le juge
    - En limitant les ressources?
  - Rédémarrant le juge une fois que celui-ci a traité une requête
  - S'interrompant si le juge quitte avec une erreur trop de fois
** Semaine du <2016-08-16 mar.> au <2016-08-19 ven.>
*** Done
- Corriger le build des images Docker de PLM-accounts et PLM-profiles
  - Refactoring de l'implémentation de [[https://github.com/jaredhanson/oauth2orize][OAuth2orize]] pour la nouvelle version de MEAN.JS
    - Auparavant, l'architecture de MEAN.JS proposée
      - Un répertoire /app/ pour le code côté serveur
      - Un répertoire /public/ pour le code côté client
    - L'implémentation actuelle se trouve donc dans ces 2 dossiers
    - Maintenant, l'architecture de MEAN.JS se base sur des modules
    - Dans chaque module se trouve
      - Un répertoire /server/ pour le code côté serveur
      - Un répertoire /client/ pour le code côté client
    - Déplacement des fichiers existants dans ces répertoires
    - Nettoyage du code
      - Côté serveur
        - Lors de la 1ère implémentation, j'avais rencontré beaucoup de difficultés à comprendre le fonctionnement de *OAuth2orize*
        - Notamment dans ce type de code
          - exports.token = [
        ...
        - Normalement, on est censé exporter une ou plusieurs méthodes ayant pour signature /function (req, res, next)/
	  - /req/ étant la requête du client
	  - /res/ l'objet servant à répondre à la requête
	  - /next/ la prochaine méthode à appeler si on a exporté plusieurs méthodes
        - Ici, les signatures des méthodes ne correspondent pas, par exemple /server.token()/
        - En fait, /server.token()/ est exécutée au lancement du serveur et renvoie en résultat une méthode ayant pour signature /function (req, res, next)/
        - C'est cette méthode générée par /server.token()/ qui sera ensuite appelée lorsqu'un utilisateur accédera à l'URL correspondante
      - Côté client
        - Redirection de l'utilisateur
	  - Lorsque l'utilisateur accède au formulaire d'autorisation pour l'accès aux données du compte par une application tierce, on vérifie s'il est connecté
	  - Si ce n'est pas le cas, on le redirige vers la page de sign-in
	  - Lorsqu'il s'est correctement authentifié, on le redirige sur la page précédente, le formulaire d'autorisation donc
	  - Cependant, si l'utilisateur ne possède pas de compte, il doit déjà s'en créer un via le formulaire de la page de sign-up
	  - Une fois son compte créé, on le redirige sur la page précédente, la page de sign-in donc, ce qui a finalement pour effet de le rediriger sur la page d'accueil
	  - *OAuth2orize* propose bien [[https://github.com/jaredhanson/connect-ensure-login][une librairie]] pour gérer les redirections mais celle-ci n'est pas compatible avec un système d'authentification utilisant des requêtes AJAX tel que celui de MEAN.JS
	  - Lors de la 1ère implémentation, j'avais alors géré les redirections manuellement à l'aide du /localStorage/ mais cette implémentation rencontrait quelques difficultés
	  - MEAN.js implémente un système pour conserver la page précédente en mémoire de façon à pouvoir rediriger l'utilisateur après que celui-ci se soit authentifié ou qu'il ait créé un compte
	  - Cependant, la transition de /sign-in/ à /sign-up/ écrase la redirection vers /oauth2/dialog/
	  - Modification de leur système pour ne pas écraser la redirection lorsqu'on navigue entre /sign-in/ et /sign-up/
  - Build de l'image Docker de PLM-accounts
    - Le build s'est déroulé correctement et l'image est fonctionnelle
    - Par contre le Dockerfile proposé par MEAN.JS est pour déployer l'application en mode développement
  - Ecriture d'un nouveau Dockerfile pour PLM-accounts
    - Image basée sur node:6
    - Installe les autres dépendances de MEAN.JS
    - Clone PLM-accounts
    - Installe ses dépendances
    - Paramètre l'application pour fonctionner en mode production
    - Démarre l'application
  - Refactoring de PLM-profiles pour la nouvelle version de MEAN.JS
    - Portage de l'ancien travail réalisé en un module MEAN.JS
    - Nettoyage du code
      - Modification de la fonction /findUniqueGitID()/ qui permet de générer un /gitID/ n'existant pas déjà dans la BDD
        - Auparavant, cette méthode était appelée manuellement lors de la création d'un profil
	- Si le /gitID/ généré aléatoirement existait déjà, cette méthode déclenchait une erreur et stoppait la création du profil
	- Dorénavant, cette méthode est appelée automatiquement lors de la création du profil via un hook de *Mongoose*
	- Boucle jusqu'à la génération d'un /gitID/ inédit
  - Build de l'image Docker de PLM-profiles
    - Le build s'est déroulé correctement et l'image est fonctionnelle
    - Pareil que pour PLM-accounts, le Dockerfile proposé par MEAN.JS est pour déployer l'application en mode développement
- Mettre à jour webPLM pour correspondre à la nouvelle version de PLM-accounts
  - Mise à jour des routes
    - Côté client
      - Route de la fenêtre de dialogue pour autoriser l'application à accéder à nos informations
    - Côté serveur
      - Route pour échanger le code d'autorisation fourni par le client contre un token
      - Route pour obtenir les informations de l'utilisateur à partir d'un token
  - Modification de la requête pour obtenir les informations de l'utilisateur
    - Ajout du header /AUTHORIZATION/ ayant pour valeur /Bearer <token>/ 
  - Problème pour tester avec différentes configurations de PLM-accounts
    - Par défaut, webPLM utilise PLM-accounts hébergé sur la machine de TN
    - La configuration par défaut est surchargeable à l'aide du fichier de config /silhouette.conf/
    - Sauf
      - côté serveur, /apiURL/ qui n'est pas un paramètre pris en compte dans la version 3 de *Silhouette*
      - côté client, plusieurs paramètres sont codés en dur dans le fichier de config /app.auth.js/ pour chaque provider
	- /authorizationEndpoint/
	- /clientID/
    - Il faut donc modifier le code selon la configuration de l'environnement de dev/test/prod
  - Surcharger /apiURL/
    - Comme dit précédemment, ce paramètre de configuration n'est pas pris en compte dans la version 3 de *Silhouette*
    - Mais il l'est dans la version 4
    - Cependant, malgré un [[http://silhouette.mohiva.com/v4.0/docs/migration-guide][guide de migration plutôt léger]], j'ai préféré ne pas mettre *Silhouette*
      - La mise à jour de *Silhouette* semble modifier plus de contenu qu'indiquer dans le guide de migration
      - *SilhouetteModule*, *ApplicationController* et *SocialAuthController* doivent être modifiés
    - Le temps manquant et les changements ne me paraissant pas clair, j'ai préféré conserver version actuelle
    - Modification de *PLMAccountsProvider*
      - C'est cet objet qui se charge d'interroger PLM-accounts et de récupérer les données de l'utilisateur
      - Il dispose d'un objet *Settings* fourni par *Silhouette*
	- C'est ici qu'on retrouve la configuration OAuth pour ce provider
	  - /authorizationEndpoint/
	  - /accessTokenURL/
	  - /clientID/
	  - /clientSecret/
        - Cet objet *Settings* dispose aussi d'un champ /customProperties/ de type *Map(String -> String)*
	- Ajout du couple /("apiURL", configuration.getString("plmaccounts.apiURL"))/ dans les /customProperties/
	- Modification de *PLMAccountsProvider* pour utiliser cette propriété pour récupérer l'/apiURL/
  - Surcharger les configurations des providers côté client
    - Jusqu'à maintenant, j'avais contourné le problème de façon dégueulasse
      - Au moment d'afficher la page HTML, le serveur injectait les valeurs de configuration des providers sous la forme de variables globales JS
      - Ces variables étaient ensuite utilisées dans /app.auth.js/ pour établir la configuration initiale des providers
    - Création d'un service côté serveur délivrant au client le JSON de configuration des providers
      - JSON obtenu à partir des valeurs renseignées dans /silhouette.conf/ et les variables d'environnement
    - Création d'un service côté client /oauthConfig/
      - Interroge le service fourni par le serveur pour récupérer la configuration des providers
      - Surcharge la configuration initiale définie dans /app.auth.js/ avec ces nouvelles valeurs
*** Planned
**** DONE Mettre à jour webPLM pour correspondre à la nouvelle version de PLM-accounts
- Au cours du refactoring de PLM-accounts, j'ai modifié certaines routes pour qu'elles soient plus logiques et pertinentes
- J'ai aussi modifié la façon dont on s'authentifie auprès de PLM-accounts pour récupérer les données d'un utilisateur
  - Auparavant, le token était passé en paramètre de la requête
  - Dorénavant le token est envoyé sous la forme d'un header
- Corriger le fonctionnement de webPLM
**** DONE Corriger le build des images Docker de PLM-accounts et PLM-profiles
- Comme dit précédemment, le build des images Docker de PLM-accounts et PLM-profiles échoue
- Des dépendances sont dépréciées
- Mettre à jour le processus de build
** Semaine du <2016-08-22 lun.> au <2016-08-26 ven.>
*** Done
- Améliorer PLM-accounts
  - Ajout du module /clients/
    - [X] Lister les applications clientes de l'utilisateur
    - [X] Créer une nouvelle application
      - [X] Nécessite un nom d'application et une URL de redirection
      - [X] Génère le clientID et le clientSecret
	- Génère un UUID pour chacun de ces champs
    - [X] Editer une application
      - [X] Changer nom et/ou URL de redirection
      - [X] Regénérer le clientID et le clientSecret
    - [X] Supprimer une application
  - Suppression du code lié à l'ancienne version de MEAN.JS
  - Suppression des modules /articles/ et /chat/
  - Page d'accueil retravaillée
  - Amélioration de la fenêtre de dialogue
    - Ajout du nom de l'utilisateur
    - Ajout du nom de l'application une fois la vérification effectuée
	...
	- Aucune application cliente enregistrée ne correspond aux paramètres reçus
    - Récupère les paramètres depuis l'URL
  - Sauvegarde de la décision de l'utilisateur
    - L'utilisateur autorise l'application cliente à accéder aux informations de son compte par le biais du formulaire d'autorisation
    - Lors de sa prochaine connexion, on saute cette étape pour passer directement à la génération du code
  - Suppression du code lié à l'authentification via OAuth
    - La gestion des comptes utilisateurs de *MEAN.JS* propose par défaut de s'authentifier via OAuth ou de lier des comptes tiers à son compte
    - Suppression de ces fonctionnalités
- Mettre en place ou utiliser un mail provider
  - Création d'un compte chez *mailgun*
    - Il est nécessaire de modifier la configuration des DNS pour envoyer des e-mails via *mailgun*
      - On doit ajouter les /DNS Records/, aussi appelé /TXT Records/ ou /SPF/ et /DKIM/
    - N'ayant pas accès au DNS provider utilisé par TN, j'ai préféré essayé *SendGrid* avant de contacter le service info de TN
  - Création d'un compte chez *SendGrid*
    - Une documentation est disponible pour envoyer des mails en utilisant la librairie *nodemailer* et *SendGrid*
      - https://sendgrid.com/blog/sending-email-nodemailer-sendgrid/
    - Définition des variables d'environnement utilisées par MEAN.JS
      - /export MAILER_FROM="PLM-accounts <do-not-reply@plm-accounts.telecomnancy.univ-lorraine.fr>"/
      - /export MAILER_SERVICE_PROVIDER=SendGrid/
      - /export MAILER_EMAIL_ID=<nom de compte>/
      - /export MAILER_PASSWORD=<mot de passe>/
    - Test de la fonctionnalité de récupération de mot de passe
    - Les mails m'ont correctement été transmis
- Déployer les nouvelles versions de PLM-accounts et PLM-profiles
  - [X] Corriger la configuration de PLM-profiles
  - [X] Ecrire la documentation et pusher les fichiers /Dockerfile/ utilisés pour PLM-accounts et PLM-profiles
    - [X] PLM-accounts
    - [X] PLM-profiles
  - [X] Pusher les images Docker
    - [X] PLM-accounts
    - [X] PLM-profiles
  - [X] Transférer les données des anciennes bases de données aux nouvelles
    - [X] /plmaccounts/ -> /plm-accounts/
    - [X] /plmprofiles/ -> /plm-profiles/
  - [X] Ajouter les nouvelles variables d'environnement sur la machine de TN
    - [X] MAILER_FROM
    - [X] MAILER_SERVICE_PROVIDER
    - [X] MAILER_EMAIL_ID
    - [X] MAILER_PASSWORD
    - [X] SESSION_SECRET
  - [X] Générer les applications clientes auprès de PLM-accounts
    - [X] PLM
    - [X] PLM-Local
  - [X] Mettre à jour la configuration de webPLM
  - [X] Re-déployer les applications
- Faire un outil de suivi de progression des élèves
  - Test pour vérifier comment récupérer le contenu d'un repo via l'API de *GitHub*
    - /curl https://api.github.com/repos/buggleinc/plm-data/contents/:filepath?ref=branch/ permet de vérifier l'existence d'un fichier
    - /curl https://api.github.com/repos/buggleinc/plm-data/contents/welcome.lessons.welcome.conditions.Conditions.scala.DONE?ref=PLM135133026701cb4c442ee940a8cd465c3997e148/
      - Le fichier existe
      - Réponse de GitHub:
	- {"name":"welcome.lessons.welcome.conditions.Conditions.scala.DONE","path":"welcome.lessons.welcome.conditions.Conditions.scala.DONE","sha":"e69de29bb2d1d6434b8b29ae775ad8c2e48c5391","size":0,"url":"https://api.github.com/repos/BuggleInc/PLM-data/contents/welcome.lessons.welcome.conditions.Conditions.scala.DONE?ref=PLM135133026701cb4c442ee940a8cd465c3997e148","html_url":"https://github.com/BuggleInc/PLM-data/blob/PLM135133026701cb4c442ee940a8cd465c3997e148/welcome.lessons.welcome.conditions.Conditions.scala.DONE","git_url":"https://api.github.com/repos/BuggleInc/PLM-data/git/blobs/e69de29bb2d1d6434b8b29ae775ad8c2e48c5391","download_url":"https://raw.githubusercontent.com/BuggleInc/PLM-data/PLM135133026701cb4c442ee940a8cd465c3997e148/welcome.lessons.welcome.conditions.Conditions.scala.DONE","type":"file","content":"","encoding":"base64","_links":{"self":"https://api.github.com/repos/BuggleInc/PLM-data/contents/welcome.lessons.welcome.conditions.Conditions.scala.DONE?ref=PLM135133026701cb4c442ee940a8cd465c3997e148","git":"https://api.github.com/repos/BuggleInc/PLM-data/git/blobs/e69de29bb2d1d6434b8b29ae775ad8c2e48c5391","html":"https://github.com/BuggleInc/PLM-data/blob/PLM135133026701cb4c442ee940a8cd465c3997e148/welcome.lessons.welcome.conditions.Conditions.scala.DONE"}}
    - /curl https://api.github.com/repos/buggleinc/plm-data/contents/conditions.Conditions.scala.DNE?ref=PLM135133026701cb4c442ee940a8cd465c3997e148/
      - Le fichier n'existe pas
      - Renvoie une erreur 404
      - Réponse de GitHub:
	- {"message":"Not Found","documentation_url":"https://developer.github.com/v3"}
  - Test pour récupérer la liste des commits effectués durant une période de temps
    - /curl https://api.github.com/repos/buggleinc/plm-data/commits?sha=branch/ permet de récupérer les commits d'un repo
    - La réponse de l'API est paginée
      - Au plus 30 commits par requête
      - Un header indique à quelle URL récupérer les commits suivants/précédents
        - Header obtenu après la requête initiale:
	  - response.headers().link:  <https://api.github.com/repositories/22066165/commits?sha=PLM135133026701cb4c442ee940a8cd465c3997e148&page=2>; rel="next", <https://api.github.com/repositories/22066165/commits?sha=PLM135133026701cb4c442ee940a8cd465c3997e148&page=4>; rel="last"
	- Header obtenu après avoir contacté /next/:
	  - response.headers().link:  <https://api.github.com/repositories/22066165/commits?sha=PLM135133026701cb4c442ee940a8cd465c3997e148&page=3>; rel="next", <https://api.github.com/repositories/22066165/commits?sha=PLM135133026701cb4c442ee940a8cd465c3997e148&page=4>; rel="last", <https://api.github.com/repositories/22066165/commits?sha=PLM135133026701cb4c442ee940a8cd465c3997e148&page=1>; rel="first", <https://api.github.com/repositories/22066165/commits?sha=PLM135133026701cb4c442ee940a8cd465c3997e148&page=1>; rel="prev"
	- Header obtenu après avoir contacté /last/:
	  - response.headers().link:  <https://api.github.com/repositories/22066165/commits?sha=PLM135133026701cb4c442ee940a8cd465c3997e148&page=1>; rel="first", <https://api.github.com/repositories/22066165/commits?sha=PLM135133026701cb4c442ee940a8cd465c3997e148&page=3>; rel="prev"
    - Les paramètres /since/ et /until/ permettent de définir les commits de quelle période nous souhaitons récupérer
      - Exemple: /curl https://api.github.com/repos/buggleinc/plm-data/commits?sha=PLMed35da9ea83a21a98a606bb5f90a44c0ad0256be&since=2016-03-15T23:00:00.000Z&until=2016-03-16T23:00:00.000Z/
  - Test pour vérifier qu'une branche existe
    - /curl https://api.github.com/repos/buggleinc/plm-data/git/refs/heads/branch/ permet de vérifier l'existence d'une branche
    - /curl https://api.github.com/repos/buggleinc/plm-data/git/refs/heads/PLM135133026701cb4c442ee940a8cd465c3997e148/
      - La branche existe
      - Réponse de GitHub:
	- {"name":"welcome.lessons.welcome.conditions.Conditions.scala.DONE","path":"welcome.lessons.welcome.conditions.Conditions.scala.DONE","sha":"e69de29bb2d1d6434b8b29ae775ad8c2e48c5391","size":0,"url":"https://api.github.com/repos/BuggleInc/PLM-data/contents/welcome.lessons.welcome.conditions.Conditions.scala.DONE?ref=PLM135133026701cb4c442ee940a8cd465c3997e148","html_url":"https://github.com/BuggleInc/PLM-data/blob/PLM135133026701cb4c442ee940a8cd465c3997e148/welcome.lessons.welcome.conditions.Conditions.scala.DONE","git_url":"https://api.github.com/repos/BuggleInc/PLM-data/git/blobs/e69de29bb2d1d6434b8b29ae775ad8c2e48c5391","download_url":"https://raw.githubusercontent.com/BuggleInc/PLM-data/PLM135133026701cb4c442ee940a8cd465c3997e148/welcome.lessons.welcome.conditions.Conditions.scala.DONE","type":"file","content":"","encoding":"base64","_links":{"self":"https://api.github.com/repos/BuggleInc/PLM-data/contents/welcome.lessons.welcome.conditions.Conditions.scala.DONE?ref=PLM135133026701cb4c442ee940a8cd465c3997e148","git":"https://api.github.com/repos/BuggleInc/PLM-data/git/blobs/e69de29bb2d1d6434b8b29ae775ad8c2e48c5391","html":"https://github.com/BuggleInc/PLM-data/blob/PLM135133026701cb4c442ee940a8cd465c3997e148/welcome.lessons.welcome.conditions.Conditions.scala.DONE"}}
    - /curl https://api.github.com/repos/buggleinc/plm-data/git/refs/heads/PLMd3dba3f52643a62934f0f42ff66d8f24493cc6d7/
      - La branche n'existe pas
      - Renvoie une erreur 404
      - Réponse de GitHub:
	- {"message":"Not Found","documentation_url":"https://developer.github.com/v3"}
  - Faire le lien entre les élèves et les branches de [[https://github.com/BuggleInc/PLM-data][PLM-data]]
    - En fait, le nom de la branche est généré de la manière suivante
      - /branch = 'PLM' + sha1(gitID)/
    - À partir du /gitID/ retourné par PLM-profiles, il nous suffit donc d'appliquer /sha1/ pour obtenir le nom de la branche
    - Différentes implémentations sont proposées en JS
      - https://developer.mozilla.org/en-US/docs/Web/API/SubtleCrypto/digest
	- Utilise seulement les méthodes disponibles nativement dans le navigateur
	- Mais nécessite de coder la méthode de conversion en hexadécimal
      - https://nodejs.org/api/crypto.html#crypto_crypto_createhash_algorithm
	- Utilise la librairie /crypto/ disponible dans *Node.js*
	- Mais nécessite de rendre disponible cette librairie côté client
- Mettre à jour webPLM pour correspondre à la nouvelle version de PLM-profiles
  - Modification du code serveur concernant le message /updateUser/
    - N'attend plus comme paramètre /firstName/ et /lastName/ mais seulement /fullName/
    - Vérifie que le champ /fullName/ n'est pas vide
  - Modification de l'interface utilisateur
    - Remplace les champs /firstName/ et /lastName/ par le champ /fullName/
    - Empêche de soumettre le formulaire si le champ /fullName/ est vide
- Améliorer PLM-profiles
  - Corriger le bug du nom/prénom
    - Suppression des champs /firstName/ et /lastName/
  - Retirer les fichiers inutiles
    - Suppression du code lié à l'ancienne version de MEAN.JS
    - Suppression des modules /articles/ et /chat/
    - Le module /users/ est plus délicat à supprimer puisqu'il fournit certains objets utilisés au sein du module /core/
      - Suppression de l'ensemble du module /users/ à l'exception des services qu'il fournit
  - Écriture du README
    - Description de l'application
    - Présentation de l'API
*** Planned
**** DONE Améliorer PLM-accounts
- Retravailler la fenêtre de dialogue
  - Personnaliser la fenêtre avec le nom de l'utilisateur
  - Afficher plus d'informations sur l'application cliente
    - Image?
- Retravailler la page d'accueil
  - Utilise la page d'accueil de MEAN.JS actuellement
  - Faire une page d'accueil propre à PLM-accounts
- Permettre de gérer les applications clientes
  - Actuellement, pour ajouter une application cliente telle que PLM, il faut insérer directement les informations en BDD
  - Ajouter un module /clients/
    - Lister les applications clientes de l'utilisateur
    - Créer une nouvelle application
      - Nécessite un nom d'application et une URL de redirection
      - Génère le clientID et le clientSecret
    - Editer une application
      - Changer nom et/ou URL de redirection
      - Regénérer le clientID et le clientSecret
    - Supprimer une application
- Retirer les fichiers inutiles
  - Code lié à l'ancienne version de MEAN.JS
    - /app/
    - /public/
  - Modules non-utilisés
    - /articles/
    - /chat/
**** DONE Mettre en place ou utiliser un mail provider
- L'application PLM-accounts doit pouvoir envoyer des emails aux utilisateurs
  - Notamment en cas d'oubli du mot de passe
- [[https://github.com/nodemailer/nodemailer#using-gmail][Nodemailer]], la librairie utilisée par MEAN.JS pour envoyer des emails, déconseille aux développeurs d'utiliser Gmail comme mail provider
- Ils recommandent d'utiliser un service dédié tels que
  - http://www.mailgun.com
  - https://sendgrid.com
- Voir pour utiliser un de ces services ou équivalents
**** DONE Mettre à jour webPLM pour correspondre à la nouvelle version de PLM-profiles
- Au cours du refactoring de PLM-accounts, j'ai modifié certaines routes pour qu'elles soient plus logiques et pertinentes
- Il faut aussi changer la page qui permet de modifier son profil depuis webPLM
  - Il ne faut plus utiliser les champs /firstName/ et /lastName/ mais un champ unique /fullName/
**** DONE Déployer les nouvelles versions de PLM-accounts et PLM-profiles
- Corriger la configuration de PLM-profiles
- Ecrire la documentation et pusher les fichiers /Dockerfile/ utilisés pour PLM-accounts et PLM-profiles
- Pusher les images Docker
- Transférer les données des anciennes bases de données aux nouvelles
- Générer les applications clientes auprès de PLM-accounts
  - PLM
  - PLM-Local
- Mettre à jour la configuration de webPLM
**** DONE Améliorer PLM-profiles
- Corriger le bug du nom/prénom
  - Les utilisateurs s'authentifiant via GitHub ne possède pas les champs /firstName/ et /lastName/, uniquement /fullName/
    - Voir l'API de GitHub concernant les profils utilisateurs :https://developer.github.com/v3/users/#response
    - Voir le code de Silhouette parsant le résultat l'API GitHub : https://github.com/mohiva/play-silhouette/blob/3.0.x/silhouette/app/com/mohiva/play/silhouette/impl/providers/oauth2/GitHubProvider.scala#L97
  - Parser le champ /fullName/ pour extraire /firstName/ et /lastName/ me semble improbable
  - Utiliser uniquement le champ /fullName/
- Modifier le README
  - Décrire le but de PLM-profiles
  - Décrire l'API proposée par PLM-profiles
- Retirer les fichiers inutiles
  - Code lié à l'ancienne version de MEAN.JS
    - /app/
    - /public/
  - Modules non-utilisés
    ...
    - /chat/
    - /users/ ?
** Semaine du <2016-08-29 lun.> au <2016-09-02 ven.>
*** Done
- Faire le tri dans les PR
  - [[https://github.com/BuggleInc/webPLM/pull/20][Add exercise editor]] : pas possible à merger cette semaine
  - [[https://github.com/BuggleInc/webPLM/pull/49][Started removing PLM]] : fermée
  - [[https://github.com/BuggleInc/webPLM/pull/80][Add localStorage]] : à vérifier si elle est fonctionnelle
- Désactiver les exercices manquants
  - Suppression des exercices concernées dans le fichier décrivant la leçon /welcome/
*** Planned
**** DONE Désactiver les exercices manquants
- Certains exercices, notamment des exercices *Bat* de la leçon /welcome/ sont pas encore portés
- Mais ils apparaissent tout de même dans la liste des exercices
- Les retirer de la liste
** Du <2016-09-05 lun.> au <2016-09-16 ven.>
*** Done
- Faire un outil de suivi de progression des élèves
  - Disponible ici : https://github.com/BuggleInc/PLM-teacher
  - Réalisé en Angular 2
  - Interroge PLM-profiles pour récupérer la liste des utilisateurs dont le compte a été créé pendant le module de prépa info
    - https://github.com/BuggleInc/PLM-teacher/blob/master/app/services/profile.service.ts#L100-L106
    - Les paramètres /since/ et /until/ sont définis ici : https://github.com/BuggleInc/PLM-teacher/blob/master/app/services/profile.service.ts#L19-L20
  - Définit une liste de sessions de cours
    - https://github.com/BuggleInc/PLM-teacher/blob/master/app/mocks/mock-sessions.ts
  - Pour chaque profil
    - Si l'utilisateur a autorisé la publication de ses données
      - Vérifie la présence de l'utilisateur aux cours : https://github.com/BuggleInc/PLM-teacher/blob/master/app/services/profile.service.ts#L53-L59
      - Liste les exercices que l'utilisateur a réussi : https://github.com/BuggleInc/PLM-teacher/blob/master/app/services/profile.service.ts#L61-L75
  - La liste des profils est ensuite mise en forme dans une /data-grid/
    - https://github.com/BuggleInc/PLM-teacher/blob/master/app/rich-grid/rich-grid.component.ts
- Correction des bugs majeurs rencontrés à la rentrée
  - La version du compiler Scala embarqué par webPLM semble lever une exception de façon aléatoire
    - Certains étudiants pouvaient travailler sans problème
    - Alors que d'autres voyaient leur serveur crash à chaque compilation
    - Mettre à jour la version de Scala utilisée semble avoir résolu ce problème
  - Si l'utilisateur écrivait du code en dehors de /run()/, une exception *EntityNotFound* était générée et faisait planter le serveur
    - Catch de cette exception et ajout d'un message d'erreur spécifique
  - Les acteurs enfants, générés pour réaliser les actions, des utilisateurs n'étaient pas détruits
    - Ceux-ci ne sont pas détruit automatiquement à l'arrêt de l'acteur parent *PLMActor*
    - Envoi de messages /PoisonPill/ pour stopper les acteurs enfants à l'arrêt de *PLMActor*
- Principales autres améliorations
  - Les instructions et aides étaient incluses dans la sérialisation JSON d'un exercice
    - Il était donc nécessaire de supprimer/regénérer la sérialisation JSON d'un exercice lorsqu'on modifiait les consignes pour pouvoir vérifier le résultat
    - Suppression des instructions et aides du JSON
    - Récupération des instructions et aides de l'exercice via les ressources
  - La sérialisation de l'exercice n'était pas mise à jour automatiquement suite à la modification de l'exercice
    - Ajout d'un mécanisme à l'initialisation du serveur en mode développement pour vérifier si la sérialisation est /outdated/
    - Instancie l'exercice depuis les sources dans ce cas et actualise la sérialisation
  - Ajout du système de traduction pour les consignes des exercices
    - Transfert de /po4a/ de *PLM* à *webPLM*
    - Modification de sa configuration pour correspondre à la nouvelle arborescence
  - Ajout de la langue brésilienne
    - Activation de langue portugaise dans la configuration de *webPLM*
      - On a un problème avec la langue brésilienne directement
      - *Play* gère les langues avec ses propres objets de type *Lang*
      - Ces objets peuvent ensuite être convertis en *Locale*
      - Dans *PLM*, nous travaillons principalement avec des *Locale*
      - Lorsque l'objet *Lang("pt_br")* est converti en *Locale*, la *Locale* obtenue est *Locale("pt")* au lieu de *Locale("pt_BR")*
    - Import des traductions des missions
  - Affichage d'un message pour indiquer à l'utilisateur qu'un juge est en train de traiter sa requête
    - Auparavant, on ne pouvait pas faire la distinction entre un problème de surcharge de nos juges et une boucle infinie
      - Dans le cas d'une surcharge, aucun juge n'est disponible dans les temps impartis pour traiter notre requête, celle-ci est donc annulée
      - Dans le cas d'une boucle infinie, un juge commence à traiter la requête mais ses ressources sont monopolisées par la boucle infinie
        - Dans certains cas, il ne parvient pas à envoyer des opérations avant que le /timeout/ soit déclaré
	- Une fois le /timeout/ déclaré, on n'envoit plus les opérations au serveur pour éviter de le surcharger
    - Ajout d'un message de ACK à la réception de la requête d'exécution par le juge
    - Ajout d'un message à l'attention de l'utilisateur lors de la réception du message de ACK par le client
*** Planned
**** DONE Faire un outil de suivi de progression des élèves
- L'idée est d'avoir un outil sommaire pour suivre la progression des élèves
- Répertorier la liste des séances du module Prépa Info de la rentrée 2016 à TN
- Définir des exercices clés pour chaque séance
- Récupérer la liste des élèves
  - Le champ /created/ du profil permet de filtrer les utilisateurs grâce à la date de leur enregistrement
- Faire le lien entre les élèves et les branches de [[https://github.com/BuggleInc/PLM-data][PLM-data]]
  - Soit webPLM doit exposer sa méthode de conversion /gitID/ -> /branche distante/
  - Soit un service doit tourner sur la machine de TN et doit pouvoir récupérer la branche distante via git
- Pour chacun d'entre eux
  - Vérifier leur présence aux séances
    - L'API de GitHub sur les commits permet de récupérer les commits effectués durant une période
      - https://developer.github.com/v3/repos/commits/
  - Vérifier si les exercices clés ont été réussis
    - Checker si le fichier /exercise.DONE/ existe
    - Voir l'API de GitHub sur le contenu d'un repo
      - https://developer.github.com/v3/repos/contents/
**** TODO Faire le tri dans les PR
- On a des PRs qui trainent dans webPLM
- Merger si possible
**** TODO Améliorer PLM-profiles
- Ajouter une clé d'API
  - Actuellement, PLM-profiles n'est accessible qu'en étant connecté sur la machine l'hébergeant
  - Mais aucune sécurité n'est mise en place sur son API
  - Voir pour ajouter des vérifications
**** TODO Tester les plugins de *New Relic* pour *RabbitMQ*
- Des plugins sont disponibles pour monitorer *RabbitMQ*
  - Par exemple : https://rpm.newrelic.com/accounts/1393737/plugins/directory/95
- Voir si ceux-ci sont compatibles avec notre application
...
- [ ] Welcome
- [ ] Bat.string1
**** TODO Vérifier le problème rencontré par Ariel
- Ariel est un contributeur souhaitant traduire PLM en espagnol
- Cependant il rencontre l'erreur suivante lorsqu'il compile PLM
  - /home/ariel/PLM-master/src/plm/test/git/GitUtilsTest.java:361: error: local variable progress is accessed from within inner class; needs to be declared final
  - ~gitUtils.pushChanges(userBranch, progress, null);~
- Résoudre cette erreur
**** TODO Re-tester les différentes librairies pour un smoothScroll
- On scroll dorénavant correctement jusqu'à la représentation du monde lorsque l'élève soumet son code
- Le scroll, par défaut, est instantané
- Ceci pourrait être améliorer en utilisant une librairie de /smoothScroll/
  - https://github.com/d-oliveros/ngSmoothScroll
  - https://github.com/arnaudbreton/angular-smoothscroll
  - https://github.com/oblador/angular-scroll
**** TODO Améliorer la configuration de logback
- Auparavant, la configuration de logback était à la fois effectuée dans /conf/application.conf/ et via un fichier /conf/logback.xml/
- Depuis *Play 2.4*, la configuration via /conf/application.conf/ est dépréciée
- Retirer les lignes correspondantes
- Revoir la stratégie de log
**** TODO Retravailler l'UI de PLM
- Durant le barcamp avec LINAGORA, j'ai eu l'occasion de rencontrer et de discuter avec Jérémie Kornobis
- Il s'occupe de prototyper leurs interfaces graphiques
- Je lui ai demandé son avis concernant l'interface de PLM
- J'ai ainsi pu avoir plusieurs remarques constructives
- Notamment, la page d'exercice est trop chargée
  - L'utilisateur a trop de choses à faire sur cette page
    - Lire les instructions
    - Prendre connaissance de l'état initial du monde, de l'état objectif et de la démo
    - Potentiellement parcourir l'aide pour trouver les méthodes à utiliser
    - Résoudre l'exercice
  - La séparer en plusieurs pages permettrait d'alléger la charge pour les utilisateurs
    - Afficher au démarrage de l'exercice une page présentant uniquement les instructions et les mondes
    - Avant de passer à la page de résolution de l'exercice
  - Le bouton *RUN* n'est pas intuitif et gagnerait à être rapproché de l'éditeur de code
- Jérémie a travaillé sur un mock-up d'interface pour donner une idée de comment la retravailler et disposer les différents éléments
- J'ai aussi obtenu quelques références à lire/consulter pour retravailler l'UI
  - Si on s'oriente vers du Material Design
    - http://www.materialup.com/ : regroupe différents composants à intégrer dans son site web
    - https://www.google.com/design/spec/material-design/introduction.html : les guidelines d'une interface Material Design
    - https://www.materialpalette.com/ : palette de couleurs
  - Du côté de la gamification
    - http://www.serious-game.fr/tag/education/ : pour voir ce qu'il se fait d'autres
    - http://opengameart.org/ : des ressources à utiliser
    - http://game-icons.net/ : autre site de ressources
**** TODO Nettoyer l'univers des tests unitaires
- Auparavant, l'univers des tests unitaires fonctionnait de la manière suivante:
  - On exécutait le code de la solution sur tous les mondes à l'initialisation pour déterminer la valeur /expected/
  - On exécutait ensuite le code de l'élève pour évaluer le /result/ obtenu
  - On comparait les 2 pour vérifier si l'exercice était réussi ou non par l'élève
- Pour rapprocher le comportement de l'univers *Bat* au comportement des autres univers, je l'avais modifié pour
  - Executer le code de la solution uniquement sur le monde solution pour déterminer son /result/
  - Et pour vérifier si l'élève a réussi l'exercice, on compare son /result/ au /result/ du monde solution
- Il reste cependant des traces de l'ancien mode de fonctionnement dans l'univers *Bat*
- Le nettoyer pour faciliter sa compréhension et sa maintenabilité
- Revoir aussi le modèle côté client
...
- Garder en tête qu'il faut pouvoir décomposer les tests d'intégration afin que leur exécution sur *Travis* ne timeout pas
...
- [X] Permettre à un utilisateur d'enregistrer un nouveau client
